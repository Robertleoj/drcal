...
if args.cull_rad_off_center is not None:
    ...
    if args.cull_rad_off_center < 0 and args.where is not None:
        print(
            "--cull-rad-off-center < 0 may not be given with --where", file=sys.stderr
        )
        sys.exit(1)
...
if (
    ...
    for l in sys.stdin:
        ...
        if args.cull_left_of is not None and float(f[1]) > args.cull_left_of:
            ...
            continue
        if (
            args.cull_rad_off_center is not None
            and nps.norm2(np.array((float(f[1]), float(f[2]))) - c) < r * r
        ):
            sys.stdout.write(l)
            continue
        if args.cull_rad_off_center_board is not None:
...
observations, _, paths = mrcal.compute_chessboard_corners(
