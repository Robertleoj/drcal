...
Rt_fit = drcal.align_procrustes_points_Rt01(Tp + noise, p)
R_fit = Rt_fit[:3, :]
t_fit = Rt_fit[3, :]
...
R_fit_vectors = drcal.align_procrustes_vectors_R01(
...
testutils.confirm_equal(
    drcal.invert_Rt(drcal.Rt_from_rt(drcal.invert_rt(drcal.rt_from_Rt(Rt)))),
    Rt,
    msg="Rt/rt and invert",
)

testutils.confirm_equal(
    drcal.compose_Rt(Rt, drcal.invert_Rt(Rt)),
    nps.glue(np.eye(3), np.zeros((3,)), axis=-2),
    msg="compose_Rt",
)

testutils.confirm_equal(
    drcal.compose_rt(drcal.rt_from_Rt(Rt), drcal.invert_rt(drcal.rt_from_Rt(Rt))),
    np.zeros((6,)),
    msg="compose_rt",
)

testutils.confirm_equal(
    drcal.compose_r(drcal.r_from_R(R), -drcal.r_from_R(R)),
    np.zeros((3,)),
    msg="compose_r",
)

testutils.confirm_equal(
    drcal.identity_Rt(), nps.glue(np.eye(3), np.zeros((3,)), axis=-2), msg="identity_Rt"
)

testutils.confirm_equal(drcal.identity_rt(), np.zeros((6,)), msg="identity_rt")

testutils.confirm_equal(drcal.transform_point_Rt(Rt, p), Tp, msg="transform_point_Rt")

testutils.confirm_equal(
    drcal.transform_point_rt(drcal.rt_from_Rt(Rt), p), Tp, msg="transform_point_rt"
)

testutils.confirm_equal(
    drcal.transform_point_Rt(drcal.invert_Rt(Rt), Tp),
    p,
    msg="transform_point_Rt inverse",
)

testutils.confirm_equal(
    drcal.transform_point_rt(drcal.invert_rt(drcal.rt_from_Rt(Rt)), Tp),
    p,
    msg="transform_point_rt inverse",
)
...
######### quaternion business
testutils.confirm_equal(
    drcal.R_from_quat(drcal.quat_from_R(R)),
    R,
...
# shape (2,4,3)
RtRt = nps.cat(Rt, drcal.compose_Rt(Rt, Rt))
testutils.confirm_equal(
    drcal.R_from_quat(drcal.quat_from_R(RR)),
    RR,
...
testutils.confirm_equal(
    drcal.quat_from_R(RR.reshape(2, 1, 3, 3)).shape,
    (2, 1, 4),
    msg="quat_from_R() shape",
)
testutils.confirm_equal(
    drcal.quat_from_R(RR.reshape(2, 3, 3)).shape, (2, 4), msg="quat_from_R() shape"
)
testutils.confirm_equal(drcal.quat_from_R(R).shape, (4,), msg="quat_from_R() shape")
...
q = drcal.quat_from_R(R)
qq = drcal.quat_from_R(RR)
drcal.quat_from_R(R, out=q2)
testutils.confirm_equal(q2, q, msg="quat_from_R() in-place")
drcal.quat_from_R(RR, out=qq2)
testutils.confirm_equal(qq2, qq, msg="quat_from_R() in-place")
drcal.R_from_quat(q, out=R2)
testutils.confirm_equal(R2, drcal.R_from_quat(q), msg="R_from_quat() in-place")
drcal.R_from_quat(qq, out=RR2)
testutils.confirm_equal(RR2, drcal.R_from_quat(qq), msg="R_from_quat() in-place")
...
qtqt = drcal.qt_from_Rt(RtRt)
drcal.qt_from_Rt(Rt, out=qt2)
testutils.confirm_equal(qt2, qt, msg="qt_from_Rt() in-place")
drcal.qt_from_Rt(RtRt, out=qtqt2)
testutils.confirm_equal(qtqt2, qtqt, msg="qt_from_Rt() in-place")
drcal.Rt_from_qt(qt, out=Rt2)
testutils.confirm_equal(Rt2, drcal.Rt_from_qt(qt), msg="Rt_from_qt() in-place")
drcal.Rt_from_qt(qtqt, out=RtRt2)
testutils.confirm_equal(RtRt2, drcal.Rt_from_qt(qtqt), msg="Rt_from_qt() in-place")
...
for p0, p1 in p:
    ...
    Rt01 = drcal.align_procrustes_points_Rt01(p0, p1, weights=w)
    Rt01_python = drcal.utils._align_procrustes_points_Rt01_python(p0, p1, weights=w)
    ...
    R01 = drcal.align_procrustes_vectors_R01(v0, v1, weights=w)
    R01_python = drcal.utils._align_procrustes_vectors_R01_python(v0, v1, weights=w)
...
# should fail
testutils.confirm(
    not np.any(drcal.align_procrustes_points_Rt01(p[0, 0, :1, :], p[0, 1, :1, :])),
    msg="align_procrustes_points_Rt01() should fail with one point",
)
testutils.confirm(
    not np.any(drcal.align_procrustes_vectors_R01(p[0, 0, :1, :], p[0, 1, :1, :])),
...
# should fail
testutils.confirm(
    not np.any(drcal.align_procrustes_points_Rt01(p[0, 0, :2, :], p[0, 1, :2, :])),
    msg="align_procrustes_points_Rt01() should fail with two points",
)
testutils.confirm(
    np.any(drcal.align_procrustes_vectors_R01(p[0, 0, :2, :], p[0, 1, :2, :])),
...
R = drcal.R_aligned_to_vector(v)
R_python = drcal.utils._R_aligned_to_vector_python(v)
