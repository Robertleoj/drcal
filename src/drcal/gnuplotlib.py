...
def _getGnuplotFeatures():
    ...
    try:
        helpstring = subprocess.check_output(
            [gnuplot_executable, "--help"], stderr=subprocess.STDOUT, env=env
        ).decode()
...
def _data_dump_only(processOptions):
    ...
    def is_gp():
        ...
        return type(h) is str and re.match(r".*\.gp$", h)

    return (
        processOptions.get("dump") or processOptions.get("terminal") == "gp" or is_gp()
    )
    ...
    for k in d:
        for i in range(len(keysets)):
            ...
            raise GnuplotlibError(
                "Option '{}' not not known in any '{}' options sets".format(
                    k, [kn[1] for kn in keysets]
                )
            )
    return dicts
...
def _massageProcessOptionsAndGetCmds(processOptions):
    ...
    if (
        processOptions.get("output") is not None
        and processOptions.get("terminal") is None
    ):
    ...
    if "cmds" in processOptions:
        cmds += processOptions["cmds"]
    return cmds
...
def _massageSubplotOptionsAndGetCmds(subplotOptions):
    ...
    if Nsquare > 1:
        raise GnuplotlibError(
            "At most 1 'square...' option could be enabled. Instead I got {}".format(
                Nsquare
            )
        )
    ...
    if subplotOptions.get("3d"):
        ...
        if not "equal_3d" in features and (
            ...
            if "square_xy" in subplotOptions:
                del subplotOptions["square_xy"]
            if "square" in subplotOptions:
                del subplotOptions["square"]
    else:
    ...
    have_set_range = set()
    for axis in ("x", "x2", "y", "y2", "z", "cb"):
        if subplotOptions.get("set") and any(
            re.match(rf" *{axis}range[\s=]", s) for s in subplotOptions["set"]
        ):
            have_set_range.add(axis)
        elif subplotOptions.get("cmds") and any(
            re.search(rf"^ *set +{axis}range[\s=]", s, flags=re.M)
            for s in subplotOptions.get("cmds")
        ):
            have_set_range.add(axis)
    ...
    for axis in ("x", "y"):
        if subplotOptions.get("set") and any(
            re.match(rf" *link +{axis}", s) for s in subplotOptions["set"]
        ):
            have_set_link.add(axis)
        elif subplotOptions.get("cmds") and any(
            re.search(rf"^ *set +link +{axis}", s, flags=re.M)
            for s in subplotOptions.get("cmds")
        ):
            have_set_link.add(axis)
    ...
    for axis in ("x", "x2", "y", "y2", "z", "cb"):
        ...
        if (
            "rgbimage" in subplotOptions
            and axis == "y"
            and not any(
                ("y" + what) in subplotOptions
        ...
        if (opt_min is not None or opt_max is not None) and opt_range is not None:
            raise GnuplotlibError(
                "{0}min/{0}max and {0}range are mutually exclusive".format(axis)
            )
        ...
        if opt_range is not None:
            if not isinstance(opt_range, (list, tuple)):
                opt_range = [
                    None if x == "*" else float(x) for x in opt_range.split(":")
                ]
            if len(opt_range) != 2:
                raise GnuplotlibError(
        ...
        ranges[axis] = "set {}range [{}:{}] {}reverse".format(
            axis,
            "*" if opt_min is None else opt_min,
            "*" if opt_max is None else opt_max,
            "" if opt_inv else "no",
        )
        ranges_default[axis] = opt_min is None and opt_max is None and opt_inv is None
    ...
    for axis in ("x", "y"):
        ...
        if axis in have_set_link:
            if (
                axis in ranges
                and not ranges_default[axis]
                and axis2 in ranges
                and not ranges_default[axis2]
            ):
                raise GnuplotlibError(
    ...
    if "cmds" in subplotOptions:
        cmds += subplotOptions["cmds"]
    return cmds
...
class gnuplotlib:
    ...
    def _startgnuplot(self):
        ...
        self.gnuplotProcess = subprocess.Popen(
            cmd,
    ...
    def _receive_until_checkpoint_or_timeout(self, checkpoint, waitforever):
        ...
        while not fromerr.endswith(checkpoint):
            ...
            rlist, wlist, xlist = select.select(
                [self.gnuplotProcess.stderr], [], [], None if waitforever else 15
            )
    ...
    def _checkpoint(self, flags=""):
        ...
        if (
            self.processOptions.get("notest")
            and not waitforever
            and not final
            and not printwarnings
        ):
            return None, None
        ...
        if ignore_known_test_failures:
            r = re.compile(
                r"""^gnuplot>\s*(?:{}|e\b).*$                  # report of the actual invalid command
                               \n^\s+\^\s*$                               # ^ mark pointing to where the error happened
                               \n^.*invalid\s+command.*$""".format(  # actual 'invalid command' complaint
                    testdataunit_ascii
                ),
                re.X + re.M,
            )
    ...
    def _logEvent(self, event):
        ...
        print(
            "==== PID {} at t={:.4f}: {}".format(
                self.gnuplotProcess.pid if self.gnuplotProcess else "(none)", t, event
            ),
            file=sys.stderr,
        )
    ...
    def _plotCurveInASCII(self, curve):
        ...
        """
        return self.processOptions.get("ascii") or (
    ...
    def _sendCurve(self, curve):
        ...
        if self._plotCurveInASCII(curve):
            if curve.get("matrix"):
                np.savetxt(
                    pipe,
                    npu.glue(*curve["_data"], axis=-2).astype(np.float64, copy=False),
                    "%s",
                )
                ...
                #     np.savetxt( pipe,
                #                 npu.glue(*curve['_data'], axis=-2).transpose().astype(np.float64,copy=False),
                #                 '%s' )
                #
            ...
            npu.mv(npu.cat(*curve["_data"]), 0, -1).astype(
    ...
    def _getPlotCmd(self, curves, subplotOptions):
        ...
        if subplotOptions.get("3d") and any("axes" in curve for curve in curves):
        ...
        if "rgbimage" in subplotOptions:
            if not os.access(subplotOptions["rgbimage"], os.R_OK) or not os.path.isfile(
            ...
            plotCurveCmdsNonDataBefore.append(
                '"{0}" binary filetype=auto flipy with rgbimage title "{0}"'.format(
                    subplotOptions["rgbimage"]
                )
            )
        ...
        for curve in curves:
            ...
            if not self._plotCurveInASCII(curve):
                ...
                if curve.get("matrix"):
                    ...
                    testData_curve = (
                        " ".join(["{}".format(testdataunit_ascii)] * curve["tuplesize"])
                        + "\n"
                        + "e\n"
                    )
        ...
        cmd = basecmd + ",".join(
            plotCurveCmdsNonDataBefore + plotCurveCmds + plotCurveCmdsNonDataAfter
        )
        cmdMinimal = basecmd + ",".join(
            plotCurveCmdsNonDataBefore
    ...
    def _massageAndValidateArgs(self, curves, curveOptions_base, subplotOptions):
        ...
        if len(curves):
            curves = [
                np.array((c,)) if isinstance(c, numbers.Real) else c for c in curves
            ]
        ...
        def reformat(curve):
            ...
            if "tuplesize" in d and d["tuplesize"] < 0:
                ...
                d["_data"] = list(npu.mv(npu.atleast_dims(curve[0], -2), -1, 0))
            else:
                d["_data"] = list(curve)
            return d
        ...
        for curve in curves:
            ...
            if curve.get("histogram"):
                ...
                if "binwidth" in curve:
                    if binwidth is not None and binwidth != curve["binwidth"]:
                        raise GnuplotlibError(
                            "Histogram binwidths must all match. This is a gnuplot limitation mostly. Got: {} and {}".format(
                                binwidth, curve["binwidth"]
                            )
                        )
                    binwidth = curve["binwidth"]
            ...
            if Ndata > curve["tuplesize"]:
                raise GnuplotlibError(
                    "Got {} tuples, but the tuplesize is {}. Giving up".format(
                        Ndata, curve["tuplesize"]
                    )
                )
            ...
            if Ndata < curve["tuplesize"]:
                ...
                if Ndata + 1 == curve["tuplesize"]:
                    ...
                    raise GnuplotlibError(
                        "plot() needed {} data arrays, but only got {}".format(
                            curve["tuplesize"], Ndata
                        )
                    )
                ...
                def checkdim(idim):
                    ...
                    if dim01[idim]:
                        if dim_here != dim01[idim]:
                            raise GnuplotlibError(
                                "plot() was given mismatched tuples to plot. {} vs {}".format(
                                    dim01[idim], dim_here
                                )
                            )
                    else:
        ...
        for curve in curves:
            ...
            for args in npu.broadcast_generate(
                ...
                curve_slice["_data"] = (
                    args[:-N_options_keys] if N_options_keys else args
                )
    ...
    def plot(self, *curves, **jointOptions):
            ...
            if self.processOptions.get("terminal") == "gp":
                ...
                if "terminal" in self.processOptions:
                    self._safelyWriteToPipe(
                        "set terminal " + self.processOptions["terminal"], "terminal"
                    )
                ...
                if "output" in self.processOptions:
                    ...
                    if (
                        "terminal" not in self.processOptions
                        or is_knownInteractiveTerminal(self.processOptions["terminal"])
                    ):
                        self._safelyWriteToPipe("set output", "output")
                    else:
        ...
        def plot_process_footer():
            if self.processOptions.get("terminal") == "gp":
                ...
                # is interactive
                terminal = self.processOptions.get("terminal", self.terminal_default)
                is_non_interactive = self.processOptions.get("output")
                is_interactive = not self.processOptions.get(
                    "output"
                ) and is_knownInteractiveTerminal(terminal)
                ...
                if self.processOptions.get("wait") and not is_non_interactive:
                    self.wait()
        ...
        def ingest_joint_options(jointOptions, subplotOptions_base, curveOptions_base):
            ...
            # handling those here
            curveOptions_here, subplotOptions_here = _split_dict(
                jointOptions,
                (knownCurveOptions, "curve"),
                (knownSubplotOptions, "subplot"),
            )
        ...
        def make_subplot_data(
            subplotOptions_base, curveOptions_base, *curves, **jointOptions
        ):
            subplotOptions, curveOptions = ingest_joint_options(
                _normalize_options_dict(jointOptions),
                subplotOptions_base,
                curveOptions_base,
            )
            ...
            curves = self._massageAndValidateArgs(curves, curveOptions, subplotOptions)
        ...
        if not is_multiplot:
            ...
            subplotOptions_base, curveOptions_base = ingest_joint_options(
                _normalize_options_dict(jointOptions),
                self.subplotOptions_base,
                self.curveOptions_base,
            )
            ...
            def make_subplot_data_embedded_kwargs(subplot):
                if type(subplot[-1]) is dict:
                    ...
                    d = {}
                return make_subplot_data(
                    subplotOptions_base, curveOptions_base, *subplot, **d
                )

            subplots = [
                make_subplot_data_embedded_kwargs(subplot) for subplot in subplots
            ]
        ...
        if not self.processOptions.get("notest"):
            ...
            if self.processOptions.get("multiplot"):
                self._safelyWriteToPipe(
                    "set multiplot "
                    + (
                        self.processOptions["multiplot"]
                        if type(self.processOptions["multiplot"]) is str
                        else ""
                    )
                )
...
def add_plot_option(d, key=None, values=None, overwrite=None, **kwargs):
    ...
    if not (
        key_normalized in keysAcceptingIterable and isinstance(values, (list, tuple))
    ):
        values = (values,)
    ...
    if key_normalized not in keysAcceptingIterable:
        if len(values) > 1:
            raise GnuplotlibError(
                "plot options given multiple values for key '{}'".format(key_normalized)
            )
        if key in d or key_normalized in d:
            ...
            if (overwrite is not None) and overwrite:
                ...
                raise GnuplotlibError(
                    "plot options already have a value for key '{}'. Pass 'overwrite=False' to use the existing one of 'overwrite=True' to use the new one".format(
                        key_normalized
                    )
                )
...
if __name__ == "__main__":
    x = np.arange(101) - 50
    plot(x**2, dump=0, ascii=0)
    time.sleep(1)

    g1 = gnuplotlib(title="Parabola with error bars", _with="xyerrorbars")
    g1.plot(x**2 * 10, np.abs(x) / 10, np.abs(x) * 5, legend="Parabola", tuplesize=4)
    time.sleep(5)

    x, y = np.ogrid[-10:11, -10:11]
    plot(x**2 + y**2, title="Heat map", set="view map", _with="image", tuplesize=3)
    time.sleep(5)

    theta = np.linspace(0, 6 * np.pi, 200)
    z = np.linspace(0, 5, 200)
    g2 = gnuplotlib(_3d=True)
    g2.plot((np.cos(theta), np.sin(theta), z), (np.cos(theta), -np.sin(theta), z))
    time.sleep(60)
