...
// - mrcal_image_TYPE_t mrcal_image_TYPE_crop(mrcal_image_TYPE_t* image, in x0,
// int y0, int w, int h)
// - mrcal_image_TYPE_save (const char* filename, const mrcal_image_TYPE_t*
// image);
...
#define MRCAL_IMAGE_ACCESSORS_DEFINE(T, Tname)                        \
    static inline T* mrcal_image_##Tname##_at(                        \
        mrcal_image_##Tname##_t* image,                               \
        int x,                                                        \
        int y                                                         \
    ) {                                                               \
        return &image->data[x + y * image->stride / sizeof(T)];       \
                                                                      ...
                                                                      \
    static inline const T* mrcal_image_##Tname##_at_const(            \
        const mrcal_image_##Tname##_t* image,                         \
        int x,                                                        \
        int y                                                         \
    ) {                                                               \
        return &image->data[x + y * image->stride / sizeof(T)];       \
                                                                      ...
                                                                      \
    static inline mrcal_image_##Tname##_t mrcal_image_##Tname##_crop( \
        mrcal_image_##Tname##_t* image,                               \
        int x0,                                                       \
        int y0,                                                       \
        int w,                                                        \
...
#define MRCAL_IMAGE_SAVE_LOAD_DECLARE(T, Tname) \
    bool mrcal_image_##Tname##_save(            \
        const char* filename,                   \
        const mrcal_image_##Tname##_t* image    \
    );                                          \
    bool mrcal_image_##Tname##_load(            \
        mrcal_image_##Tname##_t* image,         \
        const char* filename                    \
    );
