...
//
// All functions are defined using the drcal_..._full() form, which supports
...
// drcal_..._full() functions, each array is followed by the strides, one per
// dimension.
//
...
#define drcal_identity_R(R) drcal_identity_R_full(R, 0, 0)
...
#define drcal_identity_r(r) drcal_identity_r_full(r, 0)
void drcal_identity_r_full(
    double* r,     // (3,) array
    int r_stride0  // in bytes. <= 0 means "contiguous"
);
...
#define drcal_identity_Rt(Rt) drcal_identity_Rt_full(Rt, 0, 0)
...
#define drcal_identity_rt(rt) drcal_identity_rt_full(rt, 0)
void drcal_identity_rt_full(
    double* rt,     // (6,) array
    int rt_stride0  // in bytes. <= 0 means "contiguous"
);
...
#define drcal_rotate_point_R(x_out, J_R, J_x, R, x_in) \
...
#define drcal_rotate_point_R_inverted(x_out, J_R, J_x, R, x_in) \
    drcal_rotate_point_R_full(                                  \
        x_out,                                                  \
        0,                                                      \
        J_R,                                                    \
...
void drcal_rotate_point_R_full(  // output
...
// In-place operation is supported; the output array may be the same as the
// input arrays to overwrite the input.
#define drcal_transform_point_Rt(x_out, J_Rt, J_x, Rt, x_in) \
    drcal_transform_point_Rt_full(                           \
        x_out,                                               \
        0,                                                   \
        J_Rt,                                                \
        0,                                                   \
...
#define drcal_transform_point_Rt_inverted(x_out, J_Rt, J_x, Rt, x_in) \
    drcal_transform_point_Rt_full(                                    \
        x_out,                                                        \
        0,                                                            \
        J_Rt,                                                         \
...
void drcal_transform_point_Rt_full(  // output
    double* x_out,                   // (3,) array
    int x_out_stride0,               // in bytes. <= 0 means "contiguous"
...
// not wanted
#define drcal_r_from_R(r, J, R) drcal_r_from_R_full(r, 0, J, 0, 0, 0, R, 0, 0)
void drcal_r_from_R_full(  // output
    double* r,             // (3,) vector
    int r_stride0,         // in bytes. <= 0 means "contiguous"
...
// The gradient dR/dr is returned in a (3,3 ,3) array J. Set to NULL if this is
// not wanted
#define drcal_R_from_r(R, J, r) drcal_R_from_r_full(R, 0, 0, J, 0, 0, 0, r, 0)
void drcal_R_from_r_full(  // outputs
    double* R,             // (3,3) array
    int R_stride0,         // in bytes. <= 0 means "contiguous"
    int R_stride1,         // in bytes. <= 0 means "contiguous"
    double* J,             // (3,3,3) array. Gradient. May be NULL
...
// In-place operation is supported; the output array may be the same as the
// input arrays to overwrite the input.
#define drcal_invert_R(R_out, R_in) drcal_invert_R_full(R_out, 0, 0, R_in, 0, 0)
void drcal_invert_R_full(  // output
...
#define drcal_rt_from_Rt(rt, J_R, Rt) \
    drcal_rt_from_Rt_full(rt, 0, J_R, 0, 0, 0, Rt, 0, 0)
void drcal_rt_from_Rt_full(  // output
    double* rt,              // (6,) vector
    int rt_stride0,          // in bytes. <= 0 means "contiguous"
...
// 0, and I do not return them
#define drcal_Rt_from_rt(Rt, J_r, rt) \
    drcal_Rt_from_rt_full(Rt, 0, 0, J_r, 0, 0, 0, rt, 0)
void drcal_Rt_from_rt_full(  // output
    double* Rt,              // (4,3) array
    int Rt_stride0,          // in bytes. <= 0 means "contiguous"
    int Rt_stride1,          // in bytes. <= 0 means "contiguous"
...
// In-place operation is supported; the output array may be the same as the
// input arrays to overwrite the input.
#define drcal_invert_Rt(Rt_out, Rt_in) \
    drcal_invert_Rt_full(Rt_out, 0, 0, Rt_in, 0, 0)
void drcal_invert_Rt_full(  // output
...
// In-place operation is supported; the output array may be the same as the
// input arrays to overwrite the input.
#define drcal_invert_rt(rt_out, dtout_drin, dtout_dtin, rt_in) \
    drcal_invert_rt_full(                                      \
        rt_out,                                                \
        0,                                                     \
        dtout_drin,                                            \
        0,                                                     \
...
#define drcal_compose_Rt(Rt_out, Rt_0, Rt_1) \
    drcal_compose_Rt_full(Rt_out, 0, 0, Rt_0, 0, 0, Rt_1, 0, 0, false, false)
#define drcal_compose_Rt_inverted0(Rt_out, Rt_0, Rt_1) \
    drcal_compose_Rt_full(Rt_out, 0, 0, Rt_0, 0, 0, Rt_1, 0, 0, true, false)
#define drcal_compose_Rt_inverted1(Rt_out, Rt_0, Rt_1) \
    drcal_compose_Rt_full(Rt_out, 0, 0, Rt_0, 0, 0, Rt_1, 0, 0, false, true)
#define drcal_compose_Rt_inverted01(Rt_out, Rt_0, Rt_1) \
    drcal_compose_Rt_full(Rt_out, 0, 0, Rt_0, 0, 0, Rt_1, 0, 0, true, true)
void drcal_compose_Rt_full(  // output
    double* Rt_out,          // (4,3) array
    int Rt_out_stride0,      // in bytes. <= 0 means "contiguous"
    int Rt_out_stride1,      // in bytes. <= 0 means "contiguous"
...
// always, so the convenience macro drcal_compose_rt() doesn't return those. The
// other macros and drcal_compose_rt_full() do report those as well
//
// In-place operation is supported; the output array may be the same as either
// of the input arrays to overwrite the input.
#define drcal_compose_rt(rt_out, dr_dr0, dr_dr1, dt_dr0, dt_dt1, rt_0, rt_1) \
    drcal_compose_rt_full(                                                   \
        rt_out,                                                              \
        0,                                                                   \
        dr_dr0,                                                              \
        0,                                                                   \
...
#define drcal_compose_rt_inverted0( \
    rt_out,                         \
    dr_dr0,                         \
    dr_dr1,                         \
    dt_dr0,                         \
...
#define drcal_compose_rt_inverted1( \
    rt_out,                         \
    dr_dr0,                         \
    dr_dr1,                         \
    dt_dr0,                         \
...
#define drcal_compose_rt_inverted01( \
    rt_out,                          \
    dr_dr0,                          \
    dr_dr1,                          \
    dt_dr0,                          \
...
// In-place operation is supported; the output array may be the same as either
// of the input arrays to overwrite the input.
#define drcal_compose_r(r_out, dr_dr0, dr_dr1, r_0, r_1) \
...
#define drcal_compose_r_inverted0(r_out, dr_dr0, dr_dr1, r_0, r_1) \
    drcal_compose_r_full(                                          \
        r_out,                                                     \
        0,                                                         \
        dr_dr0,                                                    \
...
#define drcal_compose_r_inverted1(r_out, dr_dr0, dr_dr1, r_0, r_1) \
    drcal_compose_r_full(                                          \
        r_out,                                                     \
        0,                                                         \
        dr_dr0,                                                    \
...
#define drcal_compose_r_inverted01(r_out, dr_dr0, dr_dr1, r_0, r_1) \
    drcal_compose_r_full(                                           \
        r_out,                                                      \
        0,                                                          \
        dr_dr0,                                                     \
...
void drcal_compose_r_full(  // output
...
//
// Same as drcal_compose_r() except
//
...
//                            r1,
//                            get_gradients=True)
#define drcal_compose_r_tinyr0_gradientr0(dr_dr0, r_1) \
    drcal_compose_r_tinyr0_gradientr0_full(dr_dr0, 0, 0, r_1, 0)
void drcal_compose_r_tinyr0_gradientr0_full(  // output
    double* dr_dr0,                           // (3,3) array; may be NULL
    int dr_dr0_stride0,  // in bytes. <= 0 means "contiguous"
    int dr_dr0_stride1,  // in bytes. <= 0 means "contiguous"
...
//                            np.zeros((3,),),
//                            get_gradients=True)
#define drcal_compose_r_tinyr1_gradientr1(dr_dr1, r_0) \
    drcal_compose_r_tinyr1_gradientr1_full(dr_dr1, 0, 0, r_0, 0)
void drcal_compose_r_tinyr1_gradientr1_full(  // output
    double* dr_dr1,                           // (3,3) array; may be NULL
    int dr_dr1_stride0,  // in bytes. <= 0 means "contiguous"
    int dr_dr1_stride1,  // in bytes. <= 0 means "contiguous"
...
bool drcal_align_procrustes_vectors_R01(  // out
    double* R01,
    // in
    const int N,
    // (N,3) arrays
...
bool drcal_align_procrustes_points_Rt01(  // out
    double* Rt01,
    // in
    const int N,
    // (N,3) arrays
...
void drcal_R_aligned_to_vector(  // out
    double* R,
    // in
    const double* v
);
