...
const char* const* drcal_supported_lensmodel_names(void
);  // NULL-terminated array of char* strings

// Return true if the given drcal_lensmodel_type_t specifies a valid lens model
__attribute__((unused)) static bool drcal_lensmodel_type_is_valid(
    drcal_lensmodel_type_t t
) {
    return t >= 0;
}
...
// types
#define drcal_LENSMODEL_IS_OPENCV(d) \
    (drcal_LENSMODEL_OPENCV4 <= (d) && (d) <= drcal_LENSMODEL_OPENCV12)
...
// converted to a drcal_lensmodel_t with drcal_lensmodel_from_name()
//
// This is the inverse of drcal_lensmodel_type_from_name()
const char* drcal_lensmodel_name_unconfigured(const drcal_lensmodel_t* lensmodel
);
...
// returned by drcal_lensmodel_name_unconfigured(). For models that have a
// configuration, however, the configuration values are filled-in. The resulting
// string may be converted back into a drcal_lensmodel_t by calling
// drcal_lensmodel_from_name().
//
...
//
// This is the inverse of drcal_lensmodel_from_name()
...
//
// This is the inverse of drcal_lensmodel_name_unconfigured()
drcal_lensmodel_type_t drcal_lensmodel_type_from_name(const char* name);

// Parse the full configured lens model from a lens model name string
//
// The lens mode type AND the configuration are read into a drcal_lensmodel_t
// structure, which this function returns.
//
// On error returns false with lensmodel->type set to
// drcal_LENSMODEL_INVALID_...
//
// This is the inverse of drcal_lensmodel_name()
bool drcal_lensmodel_from_name(  // output
...
// drcal_LENSMODEL_META_LIST() macro
...
int drcal_lensmodel_num_params(const drcal_lensmodel_t* lensmodel);
...
// https://drcal.secretsauce.net/lensmodels.html#lensmodel-pinhole
//
// This is a simplified special case of drcal_project(). We project N
// camera-coordinate-system points p to N pixel coordinates q
//
// if (dq_dp != NULL) we report the gradient dq/dp in a dense (N,2,3) array
// ((N,2) drcal_point3_t objects).
void drcal_project_pinhole(  // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dp,
...
// https://drcal.secretsauce.net/lensmodels.html#lensmodel-pinhole
//
// This is a simplified special case of drcal_unproject(). We unproject N 2D
...
// ((N,3) drcal_point2_t objects).
void drcal_unproject_pinhole(  // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,
...
// if (dq_dp != NULL) we report the gradient dq/dp in a dense (N,2,3) array
// ((N,2) drcal_point3_t objects).
void drcal_project_stereographic(  // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dp,
...
// if (dv_dq != NULL) we report the gradient dv/dq in a dense (N,3,2) array
// ((N,3) drcal_point2_t objects).
void drcal_unproject_stereographic(  // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,
...
// https://drcal.secretsauce.net/lensmodels.html#lensmodel-lonlat
//
// This is a simplified special case of drcal_project(). We project N
// camera-coordinate-system points p to N pixel coordinates q
//
// if (dq_dp != NULL) we report the gradient dq/dp in a dense (N,2,3) array
// ((N,2) drcal_point3_t objects).
void drcal_project_lonlat(  // output
    drcal_point2_t* q,
...
// if (dv_dq != NULL) we report the gradient dv/dq in a dense (N,3,2) array
// ((N,3) drcal_point2_t objects).
void drcal_unproject_lonlat(  // output
    drcal_point3_t* v,
...
// if (dq_dp != NULL) we report the gradient dq/dp in a dense (N,2,3) array
// ((N,2) drcal_point3_t objects).
void drcal_project_latlon(  // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dv,  // May be NULL. Each point
                            // gets a block of 2 drcal_point3_t
                            // objects
...
// if (dv_dq != NULL) we report the gradient dv/dq in a dense (N,3,2) array
// ((N,3) drcal_point2_t objects).
void drcal_unproject_latlon(  // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,  // May be NULL. Each point
                            // gets a block of 3 drcal_point2_t
                            // objects
...
//
// This is identical to drcal_lensmodel_num_params(), but takes into account the
// problem selections. Any intrinsics parameters locked down in the
// drcal_problem_selections_t do NOT count towards the optimization parameters
...
//
// This is the inverse of drcal_unpack_solver_state_vector()
void drcal_pack_solver_state_vector(  // out, in
    double* b,
...
// input array; the input is modified, and nothing is returned.
//
// This is the inverse of drcal_pack_solver_state_vector()
void drcal_unpack_solver_state_vector(  // out, in
...
// returning false
bool drcal_corresponding_icam_extrinsics(  // out
    int* icam_extrinsics,
...
//
// This is the entry point to the drcal optimization routine. The argument list
// is commented.
drcal_stats_t drcal_optimize(  // out
...
// include the full header that defines these in drcal.h, and I don't need to:
// drcal.h just needs to know that these are a structure
...
//
// The main optimization routine in drcal_optimize() searches for optimal
...
// those to drcal_optimize(), but the inputs are all read-only It is expected
// that this will be called from Python only.
bool drcal_optimizer_callback(  // out
...
bool _drcal_drt_ref_refperturbed__dbpacked(  // output
...
// drcal_problem_selections_t, the state vector may contain any of
// - The lens parameters
...
//
// int drcal_measurement_index_THING()
...
//   - regularization
//
// int drcal_num_measurements_THING()
...
int drcal_num_measurements_points(int Nobservations_point);
...
int drcal_num_measurements_points_triangulated(  // May be NULL if we don't have
                                                 // any of these
    const drcal_observation_point_triangulated_t*
        observations_point_triangulated,
    int Nobservations_point_triangulated
);
int drcal_num_measurements_points_triangulated_initial_Npoints(  // May be NULL
                                                                 // if we don't
                                                                 // have any of
                                                                 // these
    const drcal_observation_point_triangulated_t*
...
int drcal_measurement_index_regularization(  // May be NULL if we don't have any
                                             // of these
    const drcal_observation_point_triangulated_t*
        observations_point_triangulated,
    int Nobservations_point_triangulated,
...
typedef bool (*drcal_callback_sensor_link_t)(
...
// drcal.traverse_sensor_links() for details (that Python function wraps
