...
// values
#define LENSMODEL_PRINT_CFG_ELEMENT_TEMPLATE( \
    name,                                     \
    type,                                     \
    pybuildvaluecode,                         \
    PRIcode,                                  \
    ...
    "_" #name "=..."
#define LENSMODEL_PRINT_CFG_ELEMENT_FMT( \
    name,                                \
    type,                                \
    pybuildvaluecode,                    \
    PRIcode,                             \
    ...
    "_" #name "=%" PRIcode
#define LENSMODEL_PRINT_CFG_ELEMENT_VAR( \
    name,                                \
    type,                                \
    pybuildvaluecode,                    \
    PRIcode,                             \
    ...
    , config->name
#define LENSMODEL_SCAN_CFG_ELEMENT_FMT( \
    name,                               \
    type,                               \
    pybuildvaluecode,                   \
    PRIcode,                            \
    ...
    "_" #name "=%" SCNcode
#define LENSMODEL_SCAN_CFG_ELEMENT_VAR( \
    name,                               \
    type,                               \
    pybuildvaluecode,                   \
    PRIcode,                            \
    ...
    , &config->name
#define LENSMODEL_SCAN_CFG_ELEMENT_PLUS1( \
    name,                                 \
    type,                                 \
    pybuildvaluecode,                     \
    PRIcode,                              \
    ...
    +1
const char* drcal_lensmodel_name_unconfigured(
    const drcal_lensmodel_t* lensmodel
) {
    switch (lensmodel->type) {
#define CASE_STRING_NOCONFIG(s, n) \
    case drcal_##s:;               \
        return #s;
#define _CASE_STRING_WITHCONFIG(s, n, s_CONFIG_LIST) \
    case drcal_##s:;                                 \
        return #s s_CONFIG_LIST(LENSMODEL_PRINT_CFG_ELEMENT_TEMPLATE, );
#define CASE_STRING_WITHCONFIG(s, n) \
...
static int LENSMODEL_SPLINED_STEREOGRAPHIC__snprintf_model(
    char* out,
    int size,
    const drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__config_t* config
) {
    return snprintf(
        out,
        size,
        "LENSMODEL_SPLINED_"
        "STEREOGRAPHIC" drcal_LENSMODEL_SPLINED_STEREOGRAPHIC_CONFIG_LIST(
            LENSMODEL_PRINT_CFG_ELEMENT_FMT,
        )
            drcal_LENSMODEL_SPLINED_STEREOGRAPHIC_CONFIG_LIST(
                LENSMODEL_PRINT_CFG_ELEMENT_VAR,
            )
    );
}
bool drcal_lensmodel_name(
    char* out,
    int size,
    const drcal_lensmodel_t* lensmodel
) {
...
static bool LENSMODEL_SPLINED_STEREOGRAPHIC__scan_model_config(
    ...
    int pos;
    int Nelements = 0 drcal_LENSMODEL_SPLINED_STEREOGRAPHIC_CONFIG_LIST(
        LENSMODEL_SCAN_CFG_ELEMENT_PLUS1,
    );
    return Nelements ==
               sscanf(
                   ...
                   drcal_LENSMODEL_SPLINED_STEREOGRAPHIC_CONFIG_LIST(
                       LENSMODEL_SCAN_CFG_ELEMENT_FMT,
                   ) "%n" drcal_LENSMODEL_SPLINED_STEREOGRAPHIC_CONFIG_LIST(LENSMODEL_SCAN_CFG_ELEMENT_VAR, ),
                   &pos
               ) &&
           config_str[pos] == '\0';
}

const char* const* drcal_supported_lensmodel_names(
    void
) {
#define NAMESTRING_NOCONFIG(s, n) #s,
#define _NAMESTRING_WITHCONFIG(s, n, s_CONFIG_LIST) \
    #s s_CONFIG_LIST(LENSMODEL_PRINT_CFG_ELEMENT_TEMPLATE, ),
#define NAMESTRING_WITHCONFIG(s, n) \
    _NAMESTRING_WITHCONFIG(s, n, drcal_##s##_CONFIG_LIST)
...
// parses the model name AND the configuration into a drcal_lensmodel_t
// structure. On error returns false with lensmodel->type set to
// drcal_LENSMODEL_INVALID_...
bool drcal_lensmodel_from_name(
    // output
...
#define CHECK_AND_RETURN_NOCONFIG(s, n)                      \
    if (0 == strcmp(name, #s)) {                             \
        *lensmodel = (drcal_lensmodel_t){.type = drcal_##s}; \
        return true;                                         \
    }
...
#define CHECK_AND_RETURN_WITHCONFIG(s, n)                                 \
    ...
    {                                                                     \
        ...
        if (0 == strncmp(name, #s, len_s)) {                              \
            if (name[len_s] == '\0') {                                    \
                *lensmodel = (drcal_lensmodel_t                           \
                ){.type = drcal_LENSMODEL_INVALID_MISSINGCONFIG};         \
                return false;                                             \
            }                                                             \
            if (name[len_s] == '_') {                                     \
                /* found name. Now extract the config */                  \
                *lensmodel = (drcal_lensmodel_t){.type = drcal_##s};      \
                drcal_##s##__config_t* config = &lensmodel->s##__config;  \
                                                                          \
                ...
                if (s##__scan_model_config(config, config_str))           \
                    return true;                                          \
                                                                          \
                *lensmodel = (drcal_lensmodel_t                           \
                ){.type = drcal_LENSMODEL_INVALID_BADCONFIG};             \
                return false;                                             \
            }                                                             \
        }                                                                 \
    }
    ...
    if (name == NULL) {
        *lensmodel = (drcal_lensmodel_t){.type = drcal_LENSMODEL_INVALID_TYPE};
        return false;
    }
...
// drcal_LENSMODEL_INVALID_TYPE
drcal_lensmodel_type_t drcal_lensmodel_type_from_name(
    const char* name
) {
#define CHECK_AND_RETURN_NOCONFIG(s, n) \
    if (0 == strcmp(name, #s))          \
...
#define CHECK_AND_RETURN_WITHCONFIG(s, n)                              \
    ...
    {                                                                  \
        const int len_s = strlen(#s);                                  \
        if (0 == strncmp(name, #s, len_s) &&                           \
            (name[len_s] == '\0' || name[len_s] == '_'))               \
            return drcal_##s;                                          \
    }

    if (name == NULL) {
        return drcal_LENSMODEL_INVALID_TYPE;
    }
...
drcal_lensmodel_metadata_t drcal_lensmodel_metadata(
    const drcal_lensmodel_t* lensmodel
) {
    switch (lensmodel->type) {
        case drcal_LENSMODEL_SPLINED_STEREOGRAPHIC:
        case drcal_LENSMODEL_STEREOGRAPHIC:
        case drcal_LENSMODEL_LONLAT:
        case drcal_LENSMODEL_LATLON:
...
static int LENSMODEL_SPLINED_STEREOGRAPHIC__lensmodel_num_params(
    const drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__config_t* config
) {
    return
...
int drcal_lensmodel_num_params(
    const drcal_lensmodel_t* lensmodel
) {
#define CHECK_CONFIG_NPARAM_NOCONFIG(s, n)                    \
    _Static_assert(                                           \
        n >= 0,                                               \
        "no-config implies known-at-compile-time param count" \
    );
...
static int get_num_distortions_optimization_params(
    ...
    int N = drcal_lensmodel_num_params(lensmodel);
...
int drcal_num_intrinsics_optimization_params(
    drcal_problem_selections_t problem_selections,
    const drcal_lensmodel_t* lensmodel
) {
    int N =
        get_num_distortions_optimization_params(problem_selections, lensmodel);
...
int drcal_num_states(
    ...
    return drcal_num_states_intrinsics(
               Ncameras_intrinsics,
               problem_selections,
               lensmodel
           ) +
           drcal_num_states_extrinsics(
               Ncameras_extrinsics,
               problem_selections
           ) +
           drcal_num_states_frames(Nframes, problem_selections) +
           drcal_num_states_points(Npoints, Npoints_fixed, problem_selections) +
           drcal_num_states_calobject_warp(
               problem_selections,
               Nobservations_board
           );
}

static int num_regularization_terms_percamera(
    drcal_problem_selections_t problem_selections,
    const drcal_lensmodel_t* lensmodel
) {
...
int drcal_measurement_index_boards(
    ...
    return 0 + drcal_num_measurements_boards(
                   i_observation_board,
...
int drcal_num_measurements_boards(
    int Nobservations_board,
    int calibration_object_width_n,
    int calibration_object_height_n
) {
    ...
    // *2 because I have separate x and y measurements
    return Nobservations_board * calibration_object_width_n *
           calibration_object_height_n * 2;

    return drcal_measurement_index_boards(
        Nobservations_board,
        0,
        0,
        calibration_object_width_n,
...
int drcal_measurement_index_points(
    ...
    // 2: x,y measurements
    return drcal_num_measurements_boards(
               Nobservations_board,
               calibration_object_width_n,
               calibration_object_height_n
           ) +
...
int drcal_num_measurements_points(
    int Nobservations_point
) {
    // 2: x,y measurements
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: Add a test for drcal_measurement_index_points_triangulated()"
#endif
int drcal_measurement_index_points_triangulated(
    ...
    return drcal_num_measurements_boards(
           ...
           drcal_num_measurements_points(Nobservations_point) +
           drcal_num_measurements_points_triangulated_initial_Npoints(
               observations_point_triangulated,
...
#endif
int drcal_num_measurements_points_triangulated_initial_Npoints(
    // May be NULL if we don't have any of these
    const drcal_observation_point_triangulated_t*
        observations_point_triangulated,
    int Nobservations_point_triangulated,
    ...
    // Similar loop as in _drcal_num_j_nonzero(). If the data layout changes,
...
int drcal_num_measurements_points_triangulated(
    // May be NULL if we don't have any of these
    const drcal_observation_point_triangulated_t*
        observations_point_triangulated,
    int Nobservations_point_triangulated
) {
    return drcal_num_measurements_points_triangulated_initial_Npoints(
        observations_point_triangulated,
        Nobservations_point_triangulated,
        -1
    );
...
#warning "triangulated-solve: python-wrap this function"
#endif
bool drcal_decode_observation_indices_points_triangulated(
    // output
    int* iobservation0,
    int* iobservation1,
    ...
    // Similar loop as in
    // drcal_num_measurements_points_triangulated_initial_Npoints(). If the data
    ...
    *ipoint = 0;
    ...
    while (iobservation < Nobservations_point_triangulated) {
        ...
        if (imeasurement < Nmeas + Nmeas_thispoint) {
            ...
            *iobservation1 =
                m + (*iobservation0) * ((*iobservation0) + 3 - 2 * No) / 2 + 1;
...
int drcal_measurement_index_regularization(
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: this argument order is weird. Put then triangulated stuff at the end?"
#endif
    // May be NULL if we don't have any of these
    const drcal_observation_point_triangulated_t*
        observations_point_triangulated,
    ...
    if (drcal_num_measurements_regularization(
            Ncameras_intrinsics,
            Ncameras_extrinsics,
            Nframes,
            Npoints,
    ...
    return drcal_num_measurements_boards(
           ...
           drcal_num_measurements_points(Nobservations_point) +
           drcal_num_measurements_points_triangulated(
               observations_point_triangulated,
               Nobservations_point_triangulated
           );
}

int drcal_num_measurements_regularization(
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
    int Npoints,
    ...
    return Ncameras_intrinsics * num_regularization_terms_percamera(
                                     problem_selections,
                                     lensmodel
                                 ) +
...
int drcal_num_measurements(
    ...
    return drcal_num_measurements_boards(
           ...
           drcal_num_measurements_regularization(
               Ncameras_intrinsics,
               Ncameras_extrinsics,
               Nframes,
               Npoints,
...
static bool has_calobject_warp(
    drcal_problem_selections_t problem_selections,
    int Nobservations_board
) {
    return problem_selections.do_optimize_calobject_warp &&
...
int _drcal_num_j_nonzero(
    ...
    if (lensmodel->type == drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
        ...
        Nintrinsics_per_measurement = drcal_num_intrinsics_optimization_params(
            problem_selections,
            lensmodel
        );
    }
    ...
    int N = Nobservations_board *
            ((problem_selections.do_optimize_frames ? 6 : 0) +
             (problem_selections.do_optimize_extrinsics ? 6 : 0) +
             (has_calobject_warp(problem_selections, Nobservations_board)
                  ? drcal_NSTATE_CALOBJECT_WARP
                  : 0) +
             Nintrinsics_per_measurement);
    ...
    if (observations_point_triangulated != NULL &&
        Nobservations_point_triangulated > 0) {
        // Similar loop as in
        // drcal_num_measurements_points_triangulated_initial_Npoints(). If the
    ...
    if (lensmodel->type == drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
        if (problem_selections.do_apply_regularization) {
            ...
            N += Ncameras_intrinsics * 2 *
                 num_regularization_terms_percamera(
                     problem_selections,
                     lensmodel
                 );
...
static void get_sample_coeffs__cubic(
    double* ABCD,
    double* ABCDgrad,
    double x
) {
...
static void interp__cubic(
    ...
    for (int iy = 0; iy < 4; iy++) {
        for (int k = 0; k < 2; k++) {
            cinterp[iy][k] = ABCDx[0] * c[iy * stridey + 0 * stridex + k] +
    ...
    for (int k = 0; k < 2; k++) {
        out[k] = ABCDy[0] * cinterp[0][k] + ABCDy[1] * cinterp[1][k] +
                 ABCDy[2] * cinterp[2][k] + ABCDy[3] * cinterp[3][k];
    }
}
static void sample_bspline_surface_cubic(
    double* out,
    double* dout_dx,
    double* dout_dy,
    double* ABCDx_ABCDy,
...
// and apply them together
static void get_sample_coeffs__quadratic(
    double* ABC,
    double* ABCgrad,
    double x
) {
...
static void interp__quadratic(
    ...
    for (int iy = 0; iy < 3; iy++) {
        for (int k = 0; k < 2; k++) {
            cinterp[iy][k] = ABCx[0] * c[iy * stridey + 0 * stridex + k] +
                             ABCx[1] * c[iy * stridey + 1 * stridex + k] +
                             ABCx[2] * c[iy * stridey + 2 * stridex + k];
        }
    }
    for (int k = 0; k < 2; k++) {
        out[k] = ABCy[0] * cinterp[0][k] + ABCy[1] * cinterp[1][k] +
                 ABCy[2] * cinterp[2][k];
    }
}
static void sample_bspline_surface_quadratic(
    double* out,
    double* dout_dx,
    double* dout_dy,
    double* ABCx_ABCy,
...
static void _project_point_parametric(
    ...
    const drcal_point3_t* dp_drc,
    const drcal_point3_t* dp_dtc,
    const drcal_point3_t* dp_drf,
    const drcal_point3_t* dp_dtf,
    ...
    // q = uxy/uz * fxy + cxy
    if (!(lensmodel->type == drcal_LENSMODEL_PINHOLE ||
          lensmodel->type == drcal_LENSMODEL_STEREOGRAPHIC ||
          lensmodel->type == drcal_LENSMODEL_LONLAT ||
          lensmodel->type == drcal_LENSMODEL_LATLON ||
          drcal_LENSMODEL_IS_OPENCV(lensmodel->type))) {
        MSG("Unhandled lens model: %d (%s)",
            lensmodel->type,
            drcal_lensmodel_name_unconfigured(lensmodel));
        assert(0);
    }

    drcal_point3_t dq_dp[2];
    if (lensmodel->type == drcal_LENSMODEL_PINHOLE) {
        drcal_project_pinhole(q, dq_dp, p, 1, intrinsics);
    } else if (lensmodel->type == drcal_LENSMODEL_STEREOGRAPHIC) {
        drcal_project_stereographic(q, dq_dp, p, 1, intrinsics);
    } else if (lensmodel->type == drcal_LENSMODEL_LONLAT) {
        drcal_project_lonlat(q, dq_dp, p, 1, intrinsics);
    } else if (lensmodel->type == drcal_LENSMODEL_LATLON) {
        drcal_project_latlon(q, dq_dp, p, 1, intrinsics);
    } else {
        int Nintrinsics = drcal_lensmodel_num_params(lensmodel);
    ...
    if (camera_at_identity) {
        ...
        if (dq_dtframe != NULL) {
            memcpy(dq_dtframe, (double*)dq_dp, 6 * sizeof(double));
        }
    } else {
        if (dq_drcamera != NULL) {
            mul_genN3_gen33_vout(
...
void drcal_project_pinhole(
    // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dv,  // May be NULL. Each point
                            // gets a block of 2 drcal_point3_t
                            // objects
...
void drcal_unproject_pinhole(
    // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,  // May be NULL. Each point
                            // gets a block of 3
                            // drcal_point2_t objects
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (dv_dq) {
            dv_dq[3 * i + 0] = (drcal_point2_t){.x = fx_recip};
            dv_dq[3 * i + 1] = (drcal_point2_t){.y = fy_recip};
            dv_dq[3 * i + 2] = (drcal_point2_t){};
        }
    }
}
...
// camera
void drcal_project_stereographic(
    // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dv,  // May be NULL. Each point
                            // gets a block of 2 drcal_point3_t
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (dq_dv) {
            ...
            //        = fx ( [1 0 0] scale - 2 x / ()^2 * ( [x y z]/(sqrt) + [0
            //        0 1]) ) = fx ( [scale 0 0] - x scale^2/2 * ( [x y
            //        z]/mag_xyz + [0 0 1]) )
            // Let A = -scale^2/2
            //     B = A/mag_xyz
            // dqx_dv = fx ( [scale 0 0] - x scale^2/2 * [x y z]/mag_xyz - x
            // scale^2/2 [0 0 1] )
            //        = fx ( [scale 0 0] + B x * [x y z] + x A [0 0 1] )
            double A = -scale * scale / 2.;
            double B = A / mag_xyz;
            dq_dv[2 * i + 0] =
                (drcal_point3_t){.x = fx * (v[i].x * (B * v[i].x) + scale),
                                 .y = fx * (v[i].x * (B * v[i].y)),
                                 .z = fx * (v[i].x * (B * v[i].z + A))};
            dq_dv[2 * i + 1] =
                (drcal_point3_t){.x = fy * (v[i].y * (B * v[i].x)),
                                 .y = fy * (v[i].y * (B * v[i].y) + scale),
                                 .z = fy * (v[i].y * (B * v[i].z + A))};
        }
        q[i] = (drcal_point2_t){.x = v[i].x * scale * fx + cx,
                                .y = v[i].y * scale * fy + cy};
    }
}

// Compute a stereographic unprojection using a constant fxy, cxy
void drcal_unproject_stereographic(
    // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,  // May be NULL. Each point
                            // gets a block of 3
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (dv_dq) {
            dv_dq[3 * i + 0] = (drcal_point2_t){.x = 1.0 * fx_recip};
            dv_dq[3 * i + 1] = (drcal_point2_t){.y = 1.0 * fy_recip};
            dv_dq[3 * i + 2] = (drcal_point2_t){.x = -u.x / 2.0 * fx_recip,
                                                .y = -u.y / 2.0 * fy_recip};
        }
        v[i] = (drcal_point3_t){.x = u.x, .y = u.y, .z = 1. - 1. / 4. * norm2u};
    }
}

void drcal_project_lonlat(
    // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dv,  // May be NULL. Each point
                            // gets a block of 2 drcal_point3_t
    ...
    //
    // qy ~ arcsin( vy / mag(v) ) ->
    // dqy/dv = 1 / sqrt( 1 - vy^2/norm2(v) ) 1/norm2(v) (dvy/dv mag(v)  -
    // dmag(v)/dv vy)
    //        = 1 / sqrt( norm2(v) - vy^2 ) 1/mag(v)  ( [0 mag(v)   0] -
    //        v/mag(v) vy) = 1 / sqrt( norm2(v) - vy^2 ) ( [0 1 0] - v/norm2(v)
    //        vy) = 1 / sqrt( vx^2 + vz^2 ) ( [0 1 0] - v/norm2(v) vy)
    for (int i = 0; i < N; i++) {
        double norm2_xyz_recip =
            1. / (v[i].x * v[i].x + v[i].y * v[i].y + v[i].z * v[i].z);
        double mag_xyz_recip = sqrt(norm2_xyz_recip);
        double norm2_xz_recip = 1. / (v[i].x * v[i].x + v[i].z * v[i].z);
        double mag_xz_recip = sqrt(norm2_xz_recip);

        if (dq_dv) {
            dq_dv[2 * i + 0] =
                (drcal_point3_t){.x = fx * norm2_xz_recip * v[i].z,
                                 .z = -fx * norm2_xz_recip * v[i].x};
            dq_dv[2 * i + 1] = (drcal_point3_t
            ){.x = -fy * mag_xz_recip * (v[i].y * v[i].x * norm2_xyz_recip),
              .y =
                  -fy * mag_xz_recip * (v[i].y * v[i].y * norm2_xyz_recip - 1.),
              .z = -fy * mag_xz_recip * (v[i].y * v[i].z * norm2_xyz_recip)};
        }
        q[i] = (drcal_point2_t){.x = atan2(v[i].x, v[i].z) * fx + cx,
                                .y = asin(v[i].y * mag_xyz_recip) * fy + cy};
    }
}

void drcal_unproject_lonlat(
    // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,  // May be NULL. Each point
                            // gets a block of 3 drcal_point2_t
                            // objects
    ...
    for (int i = 0; i < N; i++) {
        ...
        double clon = cos(lon);
        if (dv_dq) {
            dv_dq[3 * i + 0] = (drcal_point2_t){.x = fx_recip * clat * clon,
                                                .y = -fy_recip * slat * slon};
            dv_dq[3 * i + 1] = (drcal_point2_t){.y = fy_recip * clat};
            dv_dq[3 * i + 2] = (drcal_point2_t){.x = -fx_recip * clat * slon,
                                                .y = -fy_recip * slat * clon};
        }
        v[i] = (drcal_point3_t){.x = clat * slon, .y = slat, .z = clat * clon};
    }
}

void drcal_project_latlon(
    // output
    drcal_point2_t* q,
    drcal_point3_t* dq_dv,  // May be NULL. Each point
                            // gets a block of 2 drcal_point3_t
    ...
    // copy of drcal_project_lonlat(), with swapped x/y
    for (int i = 0; i < N; i++) {
        double norm2_xyz_recip =
            1. / (v[i].x * v[i].x + v[i].y * v[i].y + v[i].z * v[i].z);
        double mag_xyz_recip = sqrt(norm2_xyz_recip);
        double norm2_yz_recip = 1. / (v[i].y * v[i].y + v[i].z * v[i].z);
        double mag_yz_recip = sqrt(norm2_yz_recip);

        if (dq_dv) {
            dq_dv[2 * i + 0] = (drcal_point3_t
            ){.x =
                  -fx * mag_yz_recip * (v[i].x * v[i].x * norm2_xyz_recip - 1.),
              .y = -fx * mag_yz_recip * (v[i].x * v[i].y * norm2_xyz_recip),
              .z = -fx * mag_yz_recip * (v[i].x * v[i].z * norm2_xyz_recip)};
            dq_dv[2 * i + 1] =
                (drcal_point3_t){.y = fy * norm2_yz_recip * v[i].z,
                                 .z = -fy * norm2_yz_recip * v[i].y};
        }
        q[i] = (drcal_point2_t){.x = asin(v[i].x * mag_xyz_recip) * fx + cx,
                                .y = atan2(v[i].y, v[i].z) * fy + cy};
    }
}

void drcal_unproject_latlon(
    // output
    drcal_point3_t* v,
    drcal_point2_t* dv_dq,  // May be NULL. Each point
                            // gets a block of 3 drcal_point2_t
    ...
    // copy of drcal_unproject_lonlat(), with swapped x/y
    double fx_recip = 1. / fx;
    double fy_recip = 1. / fy;
    for (int i = 0; i < N; i++) {
        ...
        double clon = cos(lon);
        if (dv_dq) {
            dv_dq[3 * i + 0] = (drcal_point2_t){.x = fx_recip * clat};
            dv_dq[3 * i + 1] = (drcal_point2_t){.x = -fx_recip * slat * slon,
                                                .y = fy_recip * clat * clon};
            dv_dq[3 * i + 2] = (drcal_point2_t){.x = -fx_recip * slat * clon,
                                                .y = -fy_recip * clat * slon};
        }
        v[i] = (drcal_point3_t){.x = slat, .y = clat * slon, .z = clat * clon};
    }
}

static void
_drcal_precompute_lensmodel_data_drcal_LENSMODEL_SPLINED_STEREOGRAPHIC(
    // output
    drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__precomputed_t* precomputed,
    ...
    //
    // (u_width_needed + Nknots_margin*u_interval_size)/(Nknots - 1) =
    // u_interval_size
    // ---> u_width_needed/(Nknots-1) = u_interval_size * (1 -
    // Nknots_margin/(Nknots - 1))
    ...
    if (config->order == 2) {
        ...
        if (config->Nx < 3 || config->Ny < 3) {
            MSG("Quadratic splines: absolute minimum Nx, Ny is 3. Got Nx=%d "
                "Ny=%d. Barfing out",
                config->Nx,
                config->Ny);
            assert(0);
        ...
        if (config->Nx < 4 || config->Ny < 4) {
            MSG("Cubic splines: absolute minimum Nx, Ny is 4. Got Nx=%d Ny=%d. "
                "Barfing out",
                config->Nx,
                config->Ny);
            assert(0);
    ...
    precomputed->segments_per_u =
        (config->Nx - 1 - Nknots_margin) / (u_edge_x * 2.);
}
...
bool drcal_knots_for_splined_models(
    // buffers must hold at least
    ...
    if (lensmodel->type != drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
        MSG("This function works only with the "
            "drcal_LENSMODEL_SPLINED_STEREOGRAPHIC model. '%s' passed in",
            drcal_lensmodel_name_unconfigured(lensmodel));
        return false;
    }

    drcal_projection_precomputed_t precomputed_all;
    _drcal_precompute_lensmodel_data(&precomputed_all, lensmodel);

    const drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__config_t* config =
        &lensmodel->LENSMODEL_SPLINED_STEREOGRAPHIC__config;
    const drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__precomputed_t* precomputed =
    ...
    for (int i = 0; i < config->Nx; i++) {
        ux[i] = ((double)i - (double)(config->Nx - 1) / 2.) /
                precomputed->segments_per_u;
    }
    for (int i = 0; i < config->Ny; i++) {
        uy[i] = ((double)i - (double)(config->Ny - 1) / 2.) /
                precomputed->segments_per_u;
    }
    return true;
}

static int get_Ngradients(
    const drcal_lensmodel_t* lensmodel,
    int Nintrinsics
) {
    int N = 0;
    bool has_core = modelHasCore_fxfycxcy(lensmodel);
    bool has_dense_intrinsics_grad =
        (lensmodel->type != drcal_LENSMODEL_SPLINED_STEREOGRAPHIC);
    bool has_sparse_intrinsics_grad =
        (lensmodel->type == drcal_LENSMODEL_SPLINED_STEREOGRAPHIC);
    int runlen =
        (lensmodel->type == drcal_LENSMODEL_SPLINED_STEREOGRAPHIC)
            ? (lensmodel->LENSMODEL_SPLINED_STEREOGRAPHIC__config.order + 1)
...
static void propagate_extrinsics__splined(
    // output
    drcal_point3_t* dq_deee,
    // input
    const drcal_point3_t* dp_deee,
    ...
    for (int i = 0; i < 3; i++) {
        dq_deee[0].xyz[i] = fx * (du_deee[0].xyz[i] * (1. + ddeltau_dux[0]) +
                                  ddeltau_duy[0] * du_deee[1].xyz[i]);
        dq_deee[1].xyz[i] = fy * (du_deee[1].xyz[i] * (1. + ddeltau_duy[1]) +
                                  ddeltau_dux[1] * du_deee[0].xyz[i]);
    }
}
static void propagate_extrinsics_cam0__splined(
    // output
    drcal_point3_t* dq_deee,
    // input
    const double* dux_dp,
    ...
    for (int i = 0; i < 3; i++) {
        dq_deee[0].xyz[i] = fx * (dux_dp[i] * (1. + ddeltau_dux[0]) +
                                  ddeltau_duy[0] * duy_dp[i]);
        dq_deee[1].xyz[i] = fy * (duy_dp[i] * (1. + ddeltau_duy[1]) +
                                  ddeltau_dux[1] * dux_dp[i]);
    }
}
static void _project_point_splined(
    // outputs
    drcal_point2_t* q,
    drcal_point2_t* dq_dfxy,
    ...
    // stereographic projection
    double mag_p = sqrt(p->x * p->x + p->y * p->y + p->z * p->z);
    double scale = 2.0 / (mag_p + p->z);

    drcal_point2_t u = {.x = p->x * scale, .y = p->y * scale};
    // du/dp = d/dp ( xy * scale )
    ...
    double B = A / mag_p;
    double du_dp[2][3] = {
        {p->x * (B * p->x) + scale, p->x * (B * p->y), p->x * (B * p->z + A)},
        {p->y * (B * p->x), p->y * (B * p->y) + scale, p->y * (B * p->z + A)}
    };

    double ix = u.x * segments_per_u + (double)(Nx - 1) / 2.;
    double iy = u.y * segments_per_u + (double)(Ny - 1) / 2.;

    drcal_point2_t deltau;
    ...
    if (spline_order == 3) {
        ...
        *ivar0 = 4 +  // skip the core
                 2 * ((iy0 - 1) * Nx + (ix0 - 1));

        sample_bspline_surface_cubic(
            deltau.xy,
            ddeltau_dux,
            ddeltau_duy,
            grad_ABCDx_ABCDy,
        ...
        *ivar0 = 4 +  // skip the core
        ...
        sample_bspline_surface_quadratic(
            deltau.xy,
            ddeltau_dux,
            ddeltau_duy,
            grad_ABCDx_ABCDy,
        ...
        MSG("I only support spline order==2 or 3. Somehow got %d. This is a "
            "bug. Barfing",
            spline_order);
        assert(0);
    }
    ...
    if (camera_at_identity) {
        if (dq_drcamera != NULL) {
            memset(dq_drcamera->xyz, 0, 6 * sizeof(double));
        }
        if (dq_dtcamera != NULL) {
            memset(dq_dtcamera->xyz, 0, 6 * sizeof(double));
        }
        if (dq_drframe != NULL) {
            propagate_extrinsics__splined(
                dq_drframe,
                dp_drf,
                (const double*)du_dp,
                ddeltau_dux,
        ...
        if (dq_dtframe != NULL) {
            propagate_extrinsics_cam0__splined(
                dq_dtframe,
                du_dp[0],
                du_dp[1],
                ddeltau_dux,
...
static void _propagate_extrinsics_one(
    drcal_point3_t* dp_dparam,
    const drcal_point3_t* pt_ref,
...
static void _propagate_extrinsics_one_rzero(
    drcal_point3_t* dp_dparam,
    const drcal_point3_t* pt_ref,
    const double* dtj_dparam,
    const double* d_Rj_rj
...
static void _propagate_extrinsics_one_tzero(
    drcal_point3_t* dp_dparam,
    const drcal_point3_t* pt_ref,
    const double* drj_dparam,
    const double* d_Rj_rj
...
static void _propagate_extrinsics_one_rzero_tidentity(
    drcal_point3_t* dp_dparam,
    const drcal_point3_t* pt_ref,
    const double* d_Rj_rj
) {
    dp_dparam[0] = (drcal_point3_t){.x = 1.0};
    dp_dparam[1] = (drcal_point3_t){.y = 1.0};
    dp_dparam[2] = (drcal_point3_t){.z = 1.0};
}

static void _propagate_extrinsics_one_cam0(
    drcal_point3_t* dp_rf,
    const drcal_point3_t* pt_ref,
...
static drcal_point3_t _propagate_extrinsics(
    // output
    drcal_point3_t* _dp_drc,
    drcal_point3_t* _dp_dtc,
    drcal_point3_t* _dp_drf,
    drcal_point3_t* _dp_dtf,
    drcal_point3_t** dp_drc,
    drcal_point3_t** dp_dtc,
    drcal_point3_t** dp_drf,
    drcal_point3_t** dp_dtf,
...
// This is internal to project()
static void _project_point(
    // outputs
    drcal_point2_t* q,
    drcal_point2_t* p_dq_dfxy,
    ...
    drcal_point3_t* restrict dq_drframe,
    drcal_point3_t* restrict dq_dtframe,
    drcal_calobject_warp_t* restrict dq_dcalobject_warp,
    int* restrict* dq_dintrinsics_pool_int,
    // inputs
    const drcal_point3_t* p,
    const double* restrict intrinsics,
    const drcal_lensmodel_t* lensmodel,
    const drcal_calobject_warp_t* dpt_refz_dwarp,
    ...
    if (lensmodel->type == drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
        ...
        // WARNING: if I could assume that dq_dintrinsics_pool_double!=NULL then
        // I wouldnt need to copy the context
        if (*dq_dintrinsics_pool_int != NULL) {
            *((*dq_dintrinsics_pool_int)++) = ivar0;
            memcpy(
                gradient_sparse_meta_pool,
                grad_ABCDx_ABCDy,
    ...
    if (dq_dcalobject_warp != NULL && dpt_refz_dwarp != NULL) {
        ...
        for (int i = 0; i < drcal_NSTATE_CALOBJECT_WARP; i++) {
...
// is the main internal callback in the optimizer. This operates in one of two
// modes:
//
...
static void project(
    ...
    const double* restrict intrinsics,
    const drcal_pose_t* restrict camera_rt,
    const drcal_pose_t* restrict frame_rt,
    const drcal_calobject_warp_t* restrict calobject_warp,
    ...
    //
    // (object in reference frame) -> [frame transform] -> (object in the
    // reference frame) ->
    ...
    // [0  1 ] [0  1 ]   [0      1         ]
    //
    // This transformation (and its gradients) is handled by drcal_compose_rt()
...
#pragma GCC diagnostic ignored "-Warray-bounds"
    drcal_pose_t frame_rt_validr = {.t = frame_rt->t};
#pragma GCC diagnostic pop
    if (calibration_object_width_n) {
        frame_rt_validr.r = frame_rt->r;
    }

    if (!camera_at_identity) {
        // make sure I can pass drcal_pose_t.r as an rt[] transformation
        _Static_assert(
            offsetof(drcal_pose_t, r) == 0,
            "drcal_pose_t has expected structure"
        );
        _Static_assert(
            offsetof(drcal_pose_t, t) == 3 * sizeof(double),
            "drcal_pose_t has expected structure"
        );
        drcal_compose_rt(
            _joint_rt,
    ...
    drcal_R_from_r(Rj, d_Rj_rj, joint_rt);

    drcal_point2_t* p_dq_dfxy = NULL;
        ...
        *dq_dintrinsics_nocore = NULL;
        ...
        if (has_core) {
            ...
            *dq_dfxy = &dq_dintrinsics_pool_double[ivar_pool];
            p_dq_dfxy = (drcal_point2_t*)*dq_dfxy;
            ivar_pool += Npoints * 2;
        }
        if (has_dense_intrinsics_grad) {
            *dq_dintrinsics_nocore = p_dq_dintrinsics_nocore =
    ...
    // _project_point()
    drcal_point3_t _dp_drc[3];
    drcal_point3_t _dp_dtc[3];
    drcal_point3_t _dp_drf[3];
    drcal_point3_t _dp_dtf[3];
    drcal_point3_t* dp_drc;
    drcal_point3_t* dp_dtc;
    drcal_point3_t* dp_drf;
    drcal_point3_t* dp_dtf;

    if (calibration_object_width_n == 0) {  // projecting discrete points
        drcal_point3_t p = _propagate_extrinsics(
            _dp_drc,
            _dp_dtc,
            _dp_drf,
            _dp_dtf,
        ...
        for (int y = 0; y < calibration_object_height_n; y++) {
            for (int x = 0; x < calibration_object_width_n; x++) {
                drcal_point3_t pt_ref = {
                    .x = (double)x * calibration_object_spacing,
                    .y = (double)y * calibration_object_spacing
                };
                drcal_calobject_warp_t dpt_refz_dwarp = {};

                if (calobject_warp != NULL) {
                    // Add a board warp here. I have two parameters, and they
                    // describe additive flex along the x axis and along the y
                    // axis, in that order. In each direction the flex is a
                    // parabola, with the parameter k describing the max
                    // deflection at the center. If the ends are at +- 1 I have
                    // d = k*(1 - x^2). If the ends are at (0,N-1) the
                    // equivalent expression is: d = k*( 1 - 4*x^2/(N-1)^2 +
                    // 4*x/(N-1) - 1 ) = d = 4*k*(x/(N-1) - x^2/(N-1)^2) = d =
                    // 4.*k*x*r(1. - x*r)
                    double xr =
                        (double)x / (double)(calibration_object_width_n - 1);
                    double yr =
                        (double)y / (double)(calibration_object_height_n - 1);
                    double dx = 4. * xr * (1. - xr);
                    double dy = 4. * yr * (1. - yr);
                ...
                drcal_point3_t* dq_drcamera_here =
                    dq_drcamera ? &dq_drcamera[i_pt * 2] : NULL;
                drcal_point3_t* dq_dtcamera_here =
                    dq_dtcamera ? &dq_dtcamera[i_pt * 2] : NULL;
                drcal_point3_t* dq_drframe_here =
                    dq_drframe ? &dq_drframe[i_pt * 2] : NULL;
                drcal_point3_t* dq_dtframe_here =
                    dq_dtframe ? &dq_dtframe[i_pt * 2] : NULL;
                drcal_calobject_warp_t* dq_dcalobject_warp_here =
                    dq_dcalobject_warp ? &dq_dcalobject_warp[i_pt * 2] : NULL;

                drcal_point3_t dq_dtcamera_here_dummy[2];
                drcal_point3_t dq_dtframe_here_dummy[2];
                if (dq_dcalobject_warp) {
                    ...
                    if (!dq_dtcamera_here) {
                        dq_dtcamera_here = dq_dtcamera_here_dummy;
                    }
                    if (!dq_dtframe_here) {
                        dq_dtframe_here = dq_dtframe_here_dummy;
                    }
                }
                ...
                _project_point(
                    ...
                    p_dq_dintrinsics_nocore
                        ? &p_dq_dintrinsics_nocore[2 * (Nintrinsics - 4) * i_pt]
                        : NULL,
                    gradient_sparse_meta
                        ? &gradient_sparse_meta->pool[i_pt * runlen * 2]
                        : NULL,
                    runlen,
                    dq_drcamera_here,
                    dq_dtcamera_here,
...
// NOT A PART OF THE EXTERNAL API. This is exported for the drcal python wrapper
// only
bool _drcal_project_internal(
    // out
    drcal_point2_t* q,

    // Stored as a row-first array of shape (N,2,3). Each
    // row lives in a drcal_point3_t
    ...
    for (int i = 0; i < N; i++) {
        ...
        gradient_sparse_meta_t gradient_sparse_meta = {
        };  // init to pacify compiler warning
        ...
        if (dq_dintrinsics_nocore != NULL) {
            for (int i_xy = 0; i_xy < 2; i_xy++) {
                memcpy(
                    &dq_dintrinsics[i_xy * Nintrinsics + Ncore],
...
bool drcal_project(
    ...
    // lives in a drcal_point3_t.  May be NULL
    drcal_point3_t* dq_dp,
    ...
    if (dq_dintrinsics != NULL || dq_dp != NULL) {
        ...
        if (!meta.has_gradients) {
            MSG("drcal_project(lensmodel='%s') cannot return gradients; this "
    ...
    if (dq_dintrinsics == NULL && dq_dp == NULL &&
        (drcal_LENSMODEL_IS_OPENCV(lensmodel->type) ||
         lensmodel->type == drcal_LENSMODEL_PINHOLE)) {
        _drcal_project_internal_opencv(
            q,
            NULL,
            NULL,
    ...
    drcal_projection_precomputed_t precomputed;
    _drcal_precompute_lensmodel_data(&precomputed, lensmodel);

    return _drcal_project_internal(
        q,
        dq_dp,
        dq_dintrinsics,
        p,
...
bool drcal_unproject(
    ...
    drcal_lensmodel_metadata_t meta = drcal_lensmodel_metadata(lensmodel);
    if (!meta.has_gradients) {
        MSG("drcal_unproject(lensmodel='%s') is not yet implemented: we need "
            "gradients",
            drcal_lensmodel_name_unconfigured(lensmodel));
        return false;
    }
    ...
    return _drcal_unproject_internal(
        out,
        q,
        N,
        lensmodel,
...
static void _unproject_callback(
    const double* u,
    double* x,
    double* J,
    void* _cookie
    ...
    _unproject_callback_cookie_t* cookie =
        (_unproject_callback_cookie_t*)_cookie;
    ...
    drcal_point3_t dq_dtframe[2];
    drcal_point2_t q_hypothesis;
    project(
        &q_hypothesis,
        NULL,
        NULL,
    ...
    J[0 * 2 + 0] = dq_dtframe[0].x * dv_du[0].x + dq_dtframe[0].y * dv_du[1].x +
                   dq_dtframe[0].z * dv_du[2].x;
    J[0 * 2 + 1] = dq_dtframe[0].x * dv_du[0].y + dq_dtframe[0].y * dv_du[1].y +
                   dq_dtframe[0].z * dv_du[2].y;
    J[1 * 2 + 0] = dq_dtframe[1].x * dv_du[0].x + dq_dtframe[1].y * dv_du[1].x +
                   dq_dtframe[1].z * dv_du[2].x;
    J[1 * 2 + 1] = dq_dtframe[1].x * dv_du[0].y + dq_dtframe[1].y * dv_du[1].y +
                   dq_dtframe[1].z * dv_du[2].y;
}

// NOT A PART OF THE EXTERNAL API. This is exported for the drcal python wrapper
// only
bool _drcal_unproject_internal(
    // out
    drcal_point3_t* out,
    ...
    // easy special-cases
    if (lensmodel->type == drcal_LENSMODEL_PINHOLE) {
        drcal_unproject_pinhole(out, NULL, q, N, intrinsics);
        return true;
    }
    if (lensmodel->type == drcal_LENSMODEL_STEREOGRAPHIC) {
        drcal_unproject_stereographic(out, NULL, q, N, intrinsics);
        return true;
    }
    if (lensmodel->type == drcal_LENSMODEL_LONLAT) {
        drcal_unproject_lonlat(out, NULL, q, N, intrinsics);
        return true;
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        drcal_project_stereographic(
            (drcal_point2_t*)out->xyz,
            NULL,
            &(drcal_point3_t
            ){.x = (q[i].x - cx) / fx, .y = (q[i].y - cy) / fy, .z = 1.},
            1,
            intrinsics
        );
        ...
        double norm2x = dogleg_optimize_dense2(
            out->xyz,
            2,
            2,
            _unproject_callback,
        ...
        if (norm2x / 2.0 > 1e-4) {
            if (!already_complained) {
                // MSG("WARNING: I wasn't able to precisely compute some points.
                // norm2x=%f. Returning nan for those. Will complain just once",
                //     norm2x);
            ...
            if (!model_supports_projection_behind_camera(lensmodel) &&
                out->xyz[2] < 0.0) {
                out->xyz[0] *= -1.0;
                out->xyz[1] *= -1.0;
                out->xyz[2] *= -1.0;
...
static int pack_solver_state_intrinsics(
    // out
    ...
    for (int icam_intrinsics = 0; icam_intrinsics < Ncameras_intrinsics;
         icam_intrinsics++) {
        if (problem_selections.do_optimize_intrinsics_core && Ncore) {
            const drcal_intrinsics_core_t* intrinsics_core =
                (const drcal_intrinsics_core_t*)intrinsics;
            b[i_state++] =
                intrinsics_core->focal_xy[0] / SCALE_INTRINSICS_FOCAL_LENGTH;
            b[i_state++] =
                intrinsics_core->focal_xy[1] / SCALE_INTRINSICS_FOCAL_LENGTH;
            b[i_state++] =
                intrinsics_core->center_xy[0] / SCALE_INTRINSICS_CENTER_PIXEL;
            b[i_state++] =
                intrinsics_core->center_xy[1] / SCALE_INTRINSICS_CENTER_PIXEL;
        }
...
static int pack_solver_state_intrinsics_subset_to_subset(
    // out,in
    double* b,  // subset based on problem_selections
...
static void pack_solver_state(
    ...
    const drcal_pose_t* extrinsics_fromref,  // Ncameras_extrinsics of these
    const drcal_pose_t* frames_toref,        // Nframes of these
    const drcal_point3_t* points,            // Npoints of these
    const drcal_calobject_warp_t* calobject_warp,  // 1 of these
    drcal_problem_selections_t problem_selections,
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
    ...
    i_state += pack_solver_state_intrinsics(
        b,
        intrinsics,
        lensmodel,
        problem_selections,
    ...
    if (problem_selections.do_optimize_extrinsics) {
        for (int icam_extrinsics = 0; icam_extrinsics < Ncameras_extrinsics;
             icam_extrinsics++) {
            b[i_state++] = extrinsics_fromref[icam_extrinsics].r.xyz[0] /
                           SCALE_ROTATION_CAMERA;
            b[i_state++] = extrinsics_fromref[icam_extrinsics].r.xyz[1] /
                           SCALE_ROTATION_CAMERA;
            b[i_state++] = extrinsics_fromref[icam_extrinsics].r.xyz[2] /
                           SCALE_ROTATION_CAMERA;

            b[i_state++] = extrinsics_fromref[icam_extrinsics].t.xyz[0] /
                           SCALE_TRANSLATION_CAMERA;
            b[i_state++] = extrinsics_fromref[icam_extrinsics].t.xyz[1] /
                           SCALE_TRANSLATION_CAMERA;
            b[i_state++] = extrinsics_fromref[icam_extrinsics].t.xyz[2] /
                           SCALE_TRANSLATION_CAMERA;
        }
    }
    ...
    if (problem_selections.do_optimize_frames) {
        for (int iframe = 0; iframe < Nframes; iframe++) {
            ...
            b[i_state++] =
                frames_toref[iframe].t.xyz[0] / SCALE_TRANSLATION_FRAME;
            b[i_state++] =
                frames_toref[iframe].t.xyz[1] / SCALE_TRANSLATION_FRAME;
            b[i_state++] =
                frames_toref[iframe].t.xyz[2] / SCALE_TRANSLATION_FRAME;
        }
...
void drcal_pack_solver_state_vector(
    // out, in
    ...
    i_state += pack_solver_state_intrinsics_subset_to_subset(
        b,
        lensmodel,
        problem_selections,
        Ncameras_intrinsics
    ...
    if (problem_selections.do_optimize_extrinsics) {
        for (int icam_extrinsics = 0; icam_extrinsics < Ncameras_extrinsics;
             icam_extrinsics++) {
            drcal_pose_t* extrinsics_fromref = (drcal_pose_t*)(&b[i_state]);
            ...
            b[i_state++] =
                extrinsics_fromref->t.xyz[0] / SCALE_TRANSLATION_CAMERA;
            b[i_state++] =
                extrinsics_fromref->t.xyz[1] / SCALE_TRANSLATION_CAMERA;
            b[i_state++] =
                extrinsics_fromref->t.xyz[2] / SCALE_TRANSLATION_CAMERA;
        }
    }

    if (problem_selections.do_optimize_frames) {
        for (int iframe = 0; iframe < Nframes; iframe++) {
            drcal_pose_t* frames_toref = (drcal_pose_t*)(&b[i_state]);
        ...
        for (int i_point = 0; i_point < Npoints_variable; i_point++) {
            drcal_point3_t* points = (drcal_point3_t*)(&b[i_state]);
    ...
    if (has_calobject_warp(problem_selections, Nobservations_board)) {
        drcal_calobject_warp_t* calobject_warp =
            (drcal_calobject_warp_t*)(&b[i_state]);
        b[i_state++] = calobject_warp->x2 / SCALE_CALOBJECT_WARP;
        b[i_state++] = calobject_warp->y2 / SCALE_CALOBJECT_WARP;
    }
}

static int unpack_solver_state_intrinsics(
    // out
    ...
    for (int icam_intrinsics = 0; icam_intrinsics < Ncameras_intrinsics;
         icam_intrinsics++) {
        if (problem_selections.do_optimize_intrinsics_core && Ncore) {
            intrinsics[icam_intrinsics * intrinsics_stride + 0] =
                b[i_state++] * SCALE_INTRINSICS_FOCAL_LENGTH;
            intrinsics[icam_intrinsics * intrinsics_stride + 1] =
                b[i_state++] * SCALE_INTRINSICS_FOCAL_LENGTH;
            intrinsics[icam_intrinsics * intrinsics_stride + 2] =
                b[i_state++] * SCALE_INTRINSICS_CENTER_PIXEL;
            intrinsics[icam_intrinsics * intrinsics_stride + 3] =
                b[i_state++] * SCALE_INTRINSICS_CENTER_PIXEL;
        }

        if (problem_selections.do_optimize_intrinsics_distortions) {
            for (int i = 0; i < Nintrinsics - Ncore; i++) {
                intrinsics[icam_intrinsics * intrinsics_stride + Ncore + i] =
                    b[i_state++] * SCALE_DISTORTION;
            }
        }
    }
...
// inline
static int unpack_solver_state_intrinsics_subset_to_subset(
    // in,out
    double* b,  // subset based on problem_selections
...
static int unpack_solver_state_extrinsics_one(
    // out
    drcal_pose_t* extrinsic,
...
static int unpack_solver_state_framert_one(
    // out
    drcal_pose_t* frame,
...
static int unpack_solver_state_point_one(
    // out
    drcal_point3_t* point,
...
static int unpack_solver_state_calobject_warp(
    // out
    drcal_calobject_warp_t* calobject_warp,
...
static void unpack_solver_state(
    // out
    ...
    drcal_pose_t* extrinsics_fromref,        // Ncameras_extrinsics of these
    drcal_pose_t* frames_toref,              // Nframes of these
    drcal_point3_t* points,                  // Npoints of these
    drcal_calobject_warp_t* calobject_warp,  // 1 of these
    ...
    int i_state = unpack_solver_state_intrinsics(
        intrinsics_all,
        b,
        lensmodel,
        problem_selections,
        drcal_lensmodel_num_params(lensmodel),
        Ncameras_intrinsics
    );
    ...
    if (problem_selections.do_optimize_extrinsics) {
        for (int icam_extrinsics = 0; icam_extrinsics < Ncameras_extrinsics;
             icam_extrinsics++) {
            i_state += unpack_solver_state_extrinsics_one(
                &extrinsics_fromref[icam_extrinsics],
                &b[i_state]
            );
        }
    }
    ...
    if (problem_selections.do_optimize_frames) {
        for (int iframe = 0; iframe < Nframes; iframe++) {
            i_state += unpack_solver_state_framert_one(
                &frames_toref[iframe],
                &b[i_state]
            );
        }
        for (int i_point = 0; i_point < Npoints_variable; i_point++) {
            i_state +=
                unpack_solver_state_point_one(&points[i_point], &b[i_state]);
        }
    }

    if (has_calobject_warp(problem_selections, Nobservations_board)) {
        i_state +=
            unpack_solver_state_calobject_warp(calobject_warp, &b[i_state]);
    }

    assert(i_state == Nstate_ref);
}
// Same as above, but packs/unpacks a vector instead of structures
void drcal_unpack_solver_state_vector(
    // out, in
    ...
    int i_state = unpack_solver_state_intrinsics_subset_to_subset(
        b,
        lensmodel,
        problem_selections,
        Ncameras_intrinsics
...
int drcal_state_index_intrinsics(
    int icam_intrinsics,
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
    ...
    int Nintrinsics =
        drcal_num_intrinsics_optimization_params(problem_selections, lensmodel);
    if (Nintrinsics <= 0) {
        return -1;
    }
...
int drcal_num_states_intrinsics(
    int Ncameras_intrinsics,
    drcal_problem_selections_t problem_selections,
    const drcal_lensmodel_t* lensmodel
) {
    return Ncameras_intrinsics * drcal_num_intrinsics_optimization_params(
                                     problem_selections,
                                     lensmodel
                                 );
}

int drcal_state_index_extrinsics(
    int icam_extrinsics,
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
...
int drcal_num_states_extrinsics(
    int Ncameras_extrinsics,
    drcal_problem_selections_t problem_selections
) {
    return problem_selections.do_optimize_extrinsics ? (6 * Ncameras_extrinsics)
                                                     : 0;
}

int drcal_state_index_frames(
    int iframe,
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
...
int drcal_state_index_points(
    ...
    return drcal_num_states_intrinsics(
           ...
           drcal_num_states_frames(Nframes, problem_selections) + (i_point * 3);
}

int drcal_num_states_points(
    int Npoints,
    int Npoints_fixed,
    drcal_problem_selections_t problem_selections
) {
...
int drcal_state_index_calobject_warp(
    int Ncameras_intrinsics,
    int Ncameras_extrinsics,
    int Nframes,
    int Npoints,
...
int drcal_num_states_calobject_warp(
    drcal_problem_selections_t problem_selections,
    int Nobservations_board
) {
    if (has_calobject_warp(problem_selections, Nobservations_board)) {
        return drcal_NSTATE_CALOBJECT_WARP;
    }
    return 0;
}
...
// returning false
static bool _corresponding_icam_extrinsics__check(
    const drcal_camera_index_t* icam,
    int i,
    ...
    if (icam_map_to_intrinsics[icam_extrinsics + 1] == -100) {
        ...
        MSG("Cannot compute icam_extrinsics. I don't have a vanilla "
            "calibration problem: %s observation %d has "
            "icam_intrinsics,icam_extrinsics %d,%d while I saw %d,%d "
            "previously",
            what,
            i,
            icam_map_to_intrinsics[icam_extrinsics + 1],
            icam_extrinsics,
            icam_intrinsics,
            icam_extrinsics);
        return false;
    ...
    if (icam_map_to_extrinsics[icam_intrinsics] == -100) {
        ...
        MSG("Cannot compute icam_extrinsics. I don't have a vanilla "
            ...
            i,
            icam_intrinsics,
            icam_map_to_extrinsics[icam_intrinsics],
            icam_intrinsics,
            icam_extrinsics);
        return false;
    }
...
bool drcal_corresponding_icam_extrinsics(
    // out
    int* icam_extrinsics,
    ...
    if (!(Ncameras_intrinsics == Ncameras_extrinsics ||
        ...
        MSG("Cannot compute icam_extrinsics. I don't have a vanilla "
            "calibration problem (stationary cameras, cam0 is reference)");
        return false;
    }
    ...
    for (int i = 0; i < Ncameras_intrinsics; i++) {
        icam_map_to_extrinsics[i] = -100;
    }
    for (int i = 0; i < Ncameras_extrinsics + 1; i++) {
        icam_map_to_intrinsics[i] = -100;
    }

    for (int i = 0; i < Nobservations_board; i++) {
        if (!_corresponding_icam_extrinsics__check(
                &observations_board[i].icam,
                i,
...
static bool markOutliers(
    // output, input

    // the weight stored in each drcal_point3_t.z indicates outlierness
    // on entry AND on exit. Outliers have weight < 0.0
    drcal_point3_t* observations_board_pool,
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: not const because this is where the outlier bit lives currently"
#endif
    drcal_observation_point_triangulated_t* observations_point_triangulated,
    int Nobservations_point_triangulated,
    ...
    const drcal_pose_t* rt_extrinsics_fromref,  // Ncameras_extrinsics of these
    bool verbose
) {
    ...
    *Noutliers_triangulated_point = 0;
    ...
    const int imeasurement_board0 = drcal_measurement_index_boards(
        0,
        Nobservations_board,
        Nobservations_point,
        calibration_object_width_n,
    ...
    const int imeasurement_point_triangulated0 =
        drcal_measurement_index_points_triangulated(
            0,
            Nobservations_board,
            Nobservations_point,
    ...
    const double* x_boards = &x_measurements[imeasurement_board0];
...
#define LOOP_BOARD_FEATURE()                                              \
    const drcal_observation_board_t* observation =                        \
        &observations_board[i_observation_board];                         \
...
#define LOOP_TRIANGULATED_POINT_HEADER()          \
    drcal_observation_point_triangulated_t* pt1 = \
...
#define LOOP_TRIANGULATED_POINT_FOOTER() \
    ...
    LOOP_TRIANGULATED_POINT0(true) {
        ...
        const drcal_point3_t* v0 = &pt0->px;
        const drcal_point3_t* t_r0;
        drcal_point3_t _t_r0;
        const drcal_point3_t* v0_ref;
        drcal_point3_t _v0_ref;

        const int icam_extrinsics0 = pt0->icam.extrinsics;
        if (icam_extrinsics0 >= 0) {
            const drcal_pose_t* rt_0r =
                &rt_extrinsics_fromref[icam_extrinsics0];
            ...
            drcal_rotate_point_r_inverted(_t_r0.xyz, NULL, NULL, r_0r, t_0r);
            for (int i = 0; i < 3; i++) {
                _t_r0.xyz[i] *= -1.;
            }

            drcal_rotate_point_r_inverted(
                _v0_ref.xyz,
                NULL,
                NULL,
                r_0r,
        ...
        while (true) {
            ...
            if (pt0->outlier || pt1->outlier) {
                ...
                const drcal_point3_t* v1 = &pt1->px;

                const drcal_point3_t* t_10;
                drcal_point3_t _t_10;
                const drcal_point3_t* v0_cam1;
                drcal_point3_t _v0_cam1;

                const int icam_extrinsics1 = pt1->icam.extrinsics;
                if (icam_extrinsics1 >= 0) {
                    const drcal_pose_t* rt_1r =
                        &rt_extrinsics_fromref[icam_extrinsics1];

                    v0_cam1 = &_v0_cam1;

                    if (icam_extrinsics0 >= 0) {
                        t_10 = &_t_10;
                        drcal_transform_point_rt(
                            &_t_10.xyz[0],
                            NULL,
                            NULL,
                            &rt_1r->r.xyz[0],
                ...
                if (!_drcal_triangulate_leecivera_mid2_is_convergent(
                        v1,
                        v0_cam1,
                        t_10
                    )) {
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: outliers should not be marked in this first loop. This should happen in the following loop. Putting it here breaks the logic"
#endif

                    // There are a lot of these, so I'm disabling this print for
                    // now, to avoid spamming the terminal
                    //
                    // MSG("New divergent-feature outliers found: measurement %d
                    // observation (%d,%d)",
                    //     imeasurement_point_triangulated0 +
                    //     imeasurement_point_triangulated, i0, i1);
                }
            }
            ...
            if (pt0->outlier || pt1->outlier) {
                ...
                var += x_point_triangulated[imeasurement_point_triangulated] *
    ...
    if (Nobservations_point_triangulated > 0) {
        MSG("I started with %d triangulated outliers",
            Nmeasurements_outliers_triangulated_start);
        MSG("I started with %d triangulated outliers",
            *Noutliers_triangulated_point);
    }
    ...
    LOOP_BOARD_OBSERVATION(!foundNewOutliers) {
        LOOP_BOARD_FEATURE() {
            ...
            // -> x^2 > k^2 var
            if (dx * dx > k1 * k1 * var || dy * dy > k1 * k1 * var) {
                // MSG("Feature %d looks like an outlier. x/y are %f/%f stdevs
                // off mean (assumed 0). Observed stdev: %f, limit: %f",
                //     i_pt_board,
                //     dx/sqrt(var),
                //     dy/sqrt(var),
                //     sqrt(var),
    ...
    LOOP_BOARD_OBSERVATION(true) {
        ...
        LOOP_BOARD_FEATURE() {
            ...
            // I have sigma = sqrt(var). Outliers have abs(x) > k*sigma
            // -> x^2 > k^2 var
            if (dx * dx > k0 * k0 * var || dy * dy > k0 * k0 * var) {
                *weight *= -1.0;
                (*Noutliers_board)++;
            }
        }
        ...
        if (Npt_inlier < 3) {
            MSG("WARNING: Board observation %d (icam_intrinsics=%d, "
                "icam_extrinsics=%d, iframe=%d) had almost all of its points "
                "thrown out as outliers: only %d/%d remain. CHOLMOD is about "
                "to complain about a non-positive-definite JtJ. Something is "
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: outliers not returned to the caller yet, so I simply print them out here"
#endif
                    MSG("New outliers found: measurement %d observation "
                        "(%d,%d)",
                        imeasurement_point_triangulated0 +
                            imeasurement_point_triangulated,
                        i0,
...
typedef struct {
    ...
    const drcal_point3_t*
        points;  // Npoints of these.    In the reference frame
    const drcal_calobject_warp_t*
    ...
    const drcal_observation_board_t* observations_board;
    const drcal_point3_t* observations_board_pool;
    int Nobservations_board;

    const drcal_observation_point_t* observations_point;
    const drcal_point3_t* observations_point_pool;
    int Nobservations_point;
    ...
    drcal_problem_selections_t problem_selections;
    const drcal_problem_constants_t* problem_constants;
...
static void penalty_range_normalization(
    // out
    double* penalty,
    double* dpenalty_ddistsq,
    ...
    const double maxsq = ctx->problem_constants->point_max_range *
                         ctx->problem_constants->point_max_range;
    if (distsq > maxsq) {
    ...
    const double minsq = ctx->problem_constants->point_min_range *
                         ctx->problem_constants->point_min_range;
    if (distsq < minsq) {
...
static void optimizer_callback(
    // input state
...
#define SCALE_JACOBIAN_N(col0, scale, N)           \
    ...
    drcal_pose_t camera_rt[ctx->Ncameras_extrinsics];

    drcal_calobject_warp_t calobject_warp_local = {};
    const int i_var_calobject_warp = drcal_state_index_calobject_warp(
        ctx->Ncameras_intrinsics,
        ctx->Ncameras_extrinsics,
        ctx->Nframes,
        ctx->Npoints,
        ctx->Npoints_fixed,
        ctx->Nobservations_board,
        ctx->problem_selections,
        &ctx->lensmodel
    );
    if (has_calobject_warp(ctx->problem_selections, ctx->Nobservations_board)) {
        unpack_solver_state_calobject_warp(
            &calobject_warp_local,
            &packed_state[i_var_calobject_warp]
        );
    } else if (ctx->calobject_warp != NULL) {
        calobject_warp_local = *ctx->calobject_warp;
    }

    for (int icam_intrinsics = 0; icam_intrinsics < ctx->Ncameras_intrinsics;
         icam_intrinsics++) {
        ...
        int i_var_intrinsics = drcal_state_index_intrinsics(
            icam_intrinsics,
            ctx->Ncameras_intrinsics,
            ctx->Ncameras_extrinsics,
            ctx->Nframes,
        ...
        if (Ncore) {
            if (ctx->problem_selections.do_optimize_intrinsics_core) {
                intrinsics_here[0] = packed_state[i_var_intrinsics++] *
                                     SCALE_INTRINSICS_FOCAL_LENGTH;
                intrinsics_here[1] = packed_state[i_var_intrinsics++] *
                                     SCALE_INTRINSICS_FOCAL_LENGTH;
                intrinsics_here[2] = packed_state[i_var_intrinsics++] *
                                     SCALE_INTRINSICS_CENTER_PIXEL;
                intrinsics_here[3] = packed_state[i_var_intrinsics++] *
                                     SCALE_INTRINSICS_CENTER_PIXEL;
            } else {
                memcpy(
                    intrinsics_here,
        ...
        if (ctx->problem_selections.do_optimize_intrinsics_distortions) {
            for (int i = 0; i < ctx->Nintrinsics - Ncore; i++) {
                distortions_here[i] =
                    packed_state[i_var_intrinsics++] * SCALE_DISTORTION;
            }
        } else {
            memcpy(
                distortions_here,
    ...
    for (int icam_extrinsics = 0; icam_extrinsics < ctx->Ncameras_extrinsics;
         icam_extrinsics++) {
        if (icam_extrinsics < 0) {
            continue;
        }

        const int i_var_camera_rt = drcal_state_index_extrinsics(
            icam_extrinsics,
            ctx->Ncameras_intrinsics,
            ctx->Ncameras_extrinsics,
            ctx->Nframes,
        ...
        if (ctx->problem_selections.do_optimize_extrinsics) {
            unpack_solver_state_extrinsics_one(
                &camera_rt[icam_extrinsics],
                &packed_state[i_var_camera_rt]
            );
        } else {
            memcpy(
                &camera_rt[icam_extrinsics],
                &ctx->extrinsics_fromref[icam_extrinsics],
                sizeof(drcal_pose_t)
            );
        }
    }
    ...
    for (int i_observation_board = 0;
         ...
         i_observation_board++) {
        const drcal_observation_board_t* observation =
            &ctx->observations_board[i_observation_board];
        ...
        const int i_var_frame_rt = drcal_state_index_frames(
            iframe,
            ctx->Ncameras_intrinsics,
            ctx->Ncameras_extrinsics,
            ctx->Nframes,
        ...
        drcal_pose_t frame_rt;
        if (ctx->problem_selections.do_optimize_frames) {
            unpack_solver_state_framert_one(
                &frame_rt,
                &packed_state[i_var_frame_rt]
            );
        } else {
            memcpy(&frame_rt, &ctx->frames_toref[iframe], sizeof(drcal_pose_t));
        }

        const int i_var_intrinsics = drcal_state_index_intrinsics(
            icam_intrinsics,
            ctx->Ncameras_intrinsics,
            ctx->Ncameras_extrinsics,
            ctx->Nframes,
        ...
        drcal_point3_t dq_drcamera
            [ctx->calibration_object_width_n * ctx->calibration_object_height_n]
            [2];
        drcal_point3_t dq_dtcamera
            [ctx->calibration_object_width_n * ctx->calibration_object_height_n]
            [2];
        drcal_point3_t dq_drframe
            [ctx->calibration_object_width_n * ctx->calibration_object_height_n]
            [2];
        drcal_point3_t dq_dtframe
            [ctx->calibration_object_width_n * ctx->calibration_object_height_n]
            [2];
        drcal_calobject_warp_t dq_dcalobject_warp
            [ctx->calibration_object_width_n * ctx->calibration_object_height_n]
            [2];
        drcal_point2_t q_hypothesis
        ...
        // All the data lives in dq_dintrinsics_pool_double[], with the other
        // data indicating the meaning of the values in the pool.
        //
        ...
        double dq_dintrinsics_pool_double
            [ctx->calibration_object_width_n *
             ctx->calibration_object_height_n * Ngradients];
        int dq_dintrinsics_pool_int
            [ctx->calibration_object_width_n *
             ctx->calibration_object_height_n];
        double* dq_dfxy = NULL;
        double* dq_dintrinsics_nocore = NULL;
        ...
        for (int i_pt = 0; i_pt < ctx->calibration_object_width_n *
                                      ctx->calibration_object_height_n;
             i_pt++, i_feature++) {
            const drcal_point3_t* qx_qy_w__observed =
                &ctx->observations_board_pool[i_feature];
            ...
            if (weight >= 0.0) {
                ...
                for (int i_xy = 0; i_xy < 2; i_xy++) {
                    const double err = (q_hypothesis[i_pt].xy[i_xy] -
                                        qx_qy_w__observed->xyz[i_xy]) *
                                       weight;
                    ...
                    if (ctx->problem_selections.do_optimize_intrinsics_core) {
                        ...
                        STORE_JACOBIAN(
                            ...
                            dq_dfxy[i_pt * 2 + i_xy] * weight *
                                SCALE_INTRINSICS_FOCAL_LENGTH
                        );

                        // cx,cy. The gradients here are known to be 1. And x
                        // depends on cx only. And y depends on cy only
                        STORE_JACOBIAN(
                    ...
                    if (ctx->problem_selections
                        ...
                        if (gradient_sparse_meta.pool != NULL) {
                            ...
                            const int len =
                                gradient_sparse_meta.run_side_length;
                            const double* ABCDx =
                                &gradient_sparse_meta.pool
                                     [len * 2 * splined_intrinsics_grad_irun +
                                      0];
                            const double* ABCDy =
                                &gradient_sparse_meta.pool
                                     [len * 2 * splined_intrinsics_grad_irun +
                            ...
                            const int ivar_stridey =
                                gradient_sparse_meta.ivar_stridey;
                            const double* fxy =
                                &intrinsics_all[icam_intrinsics][0];

                            for (int iy = 0; iy < len; iy++) {
                                for (int ix = 0; ix < len; ix++) {
                                    STORE_JACOBIAN(
                                        i_var_intrinsics + ivar0 +
                                            iy * ivar_stridey + ix * 2 + i_xy,
                                        ABCDx[ix] * ABCDy[iy] * fxy[i_xy] *
                            ...
                            for (int i = 0; i < ctx->Nintrinsics - Ncore; i++) {
                                STORE_JACOBIAN(
                                    i_var_intrinsics + Ncore_state + i,
                                    dq_dintrinsics_nocore
                                            [i_pt * 2 *
                                                 (ctx->Nintrinsics - Ncore) +
                    ...
                    if (ctx->problem_selections.do_optimize_extrinsics) {
                        if (icam_extrinsics >= 0) {
                            STORE_JACOBIAN3(
                                i_var_camera_rt + 0,
                                dq_drcamera[i_pt][i_xy].xyz[0] * weight *
                                    SCALE_ROTATION_CAMERA,
                                dq_drcamera[i_pt][i_xy].xyz[1] * weight *
                                    SCALE_ROTATION_CAMERA,
                                dq_drcamera[i_pt][i_xy].xyz[2] * weight *
                                    SCALE_ROTATION_CAMERA
                            );
                            STORE_JACOBIAN3(
                                i_var_camera_rt + 3,
                                dq_dtcamera[i_pt][i_xy].xyz[0] * weight *
                                    SCALE_TRANSLATION_CAMERA,
                                dq_dtcamera[i_pt][i_xy].xyz[1] * weight *
                                    SCALE_TRANSLATION_CAMERA,
                                dq_dtcamera[i_pt][i_xy].xyz[2] * weight *
                                    SCALE_TRANSLATION_CAMERA
                            );
                        }
                    }
                    ...
                    if (ctx->problem_selections.do_optimize_frames) {
                        STORE_JACOBIAN3(
                            i_var_frame_rt + 0,
                            dq_drframe[i_pt][i_xy].xyz[0] * weight *
                                SCALE_ROTATION_FRAME,
                            dq_drframe[i_pt][i_xy].xyz[1] * weight *
                                SCALE_ROTATION_FRAME,
                            dq_drframe[i_pt][i_xy].xyz[2] * weight *
                                SCALE_ROTATION_FRAME
                        );
                        STORE_JACOBIAN3(
                            i_var_frame_rt + 3,
                            dq_dtframe[i_pt][i_xy].xyz[0] * weight *
                                SCALE_TRANSLATION_FRAME,
                            dq_dtframe[i_pt][i_xy].xyz[1] * weight *
                                SCALE_TRANSLATION_FRAME,
                            dq_dtframe[i_pt][i_xy].xyz[2] * weight *
                                SCALE_TRANSLATION_FRAME
                        );
                    }

                    if (has_calobject_warp(
                            ctx->problem_selections,
                            ctx->Nobservations_board
                        )) {
                        STORE_JACOBIAN_N(
                            i_var_calobject_warp,
    ...
    for (int i_observation_point = 0;
         ...
         i_observation_point++) {
        const drcal_observation_point_t* observation =
            &ctx->observations_point[i_observation_point];
        ...
        const drcal_point3_t* qx_qy_w__observed =
            &ctx->observations_point_pool[i_observation_point];
        double weight = qx_qy_w__observed->z;
        ...
        if (weight <= 0.0) {
            ...
            const int i_var_point = drcal_state_index_points(
                i_point,
                ctx->Ncameras_intrinsics,
                ctx->Ncameras_extrinsics,
                ctx->Nframes,
                ...
                if (ctx->problem_selections
                    ...
                    if ((ctx->problem_selections.do_optimize_intrinsics_core ||
                         ctx->problem_selections
                             .do_optimize_intrinsics_distortions) &&
                        ctx->lensmodel.type ==
                            drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
                        ...
                        const drcal_LENSMODEL_SPLINED_STEREOGRAPHIC__config_t*
                            config =
                                &ctx->lensmodel
        ...
        drcal_point3_t point_ref;
        if (use_position_from_state) {
            unpack_solver_state_point_one(
                &point_ref,
                &packed_state[i_var_point]
            );
        } else {
        ...
        // WARNING: "compute size(dq_dintrinsics_pool_double) correctly and
        // maybe bounds-check"
        double dq_dintrinsics_pool_double[Ngradients];
        ...
        drcal_point3_t dq_drcamera[2];
        drcal_point3_t dq_dtcamera[2];
        drcal_point3_t dq_dpoint[2];
...
#pragma GCC diagnostic ignored "-Warray-bounds"
        ...
        project(
            ...
            ctx->problem_selections.do_optimize_extrinsics ? dq_drcamera : NULL,
            ctx->problem_selections.do_optimize_extrinsics ? dq_dtcamera : NULL,
...
#pragma GCC diagnostic pop
        ...
        for (int i_xy = 0; i_xy < 2; i_xy++) {
            const double err =
                (q_hypothesis.xy[i_xy] - qx_qy_w__observed->xyz[i_xy]) * weight;
            ...
            if (ctx->problem_selections.do_optimize_intrinsics_core) {
                ...
                STORE_JACOBIAN(
                    i_var_intrinsics + i_xy,
                    dq_dfxy[i_xy] * weight * SCALE_INTRINSICS_FOCAL_LENGTH
                );

                // cx,cy. The gradients here are known to be 1. And x depends on
                // cx only. And y depends on cy only
                STORE_JACOBIAN(
                    i_var_intrinsics + i_xy + 2,
                    weight * SCALE_INTRINSICS_CENTER_PIXEL
                );
            ...
            if (ctx->problem_selections.do_optimize_intrinsics_distortions) {
                if (gradient_sparse_meta.pool != NULL) {
                    ...
                    for (int iy = 0; iy < len; iy++) {
                        for (int ix = 0; ix < len; ix++) {
                            STORE_JACOBIAN(
                                i_var_intrinsics + ivar0 + iy * ivar_stridey +
                                    ix * 2 + i_xy,
                                ABCDx[ix] * ABCDy[iy] * fxy[i_xy] * weight *
                                    SCALE_DISTORTION
                            );
                        }
                    }
            ...
            if (ctx->problem_selections.do_optimize_extrinsics) {
                if (icam_extrinsics >= 0) {
                    STORE_JACOBIAN3(
                        i_var_camera_rt + 0,
                        dq_drcamera[i_xy].xyz[0] * weight *
                            SCALE_ROTATION_CAMERA,
                        dq_drcamera[i_xy].xyz[1] * weight *
                            SCALE_ROTATION_CAMERA,
                        dq_drcamera[i_xy].xyz[2] * weight *
                            SCALE_ROTATION_CAMERA
                    );
                    STORE_JACOBIAN3(
                        i_var_camera_rt + 3,
                        dq_dtcamera[i_xy].xyz[0] * weight *
                            SCALE_TRANSLATION_CAMERA,
                        dq_dtcamera[i_xy].xyz[1] * weight *
                            SCALE_TRANSLATION_CAMERA,
                        dq_dtcamera[i_xy].xyz[2] * weight *
                            SCALE_TRANSLATION_CAMERA
                    );
                }
            }
            ...
            if (use_position_from_state) {
                STORE_JACOBIAN3(
                    i_var_point,
                    dq_dpoint[i_xy].xyz[0] * weight * SCALE_POSITION_POINT,
    ...
    if (ctx->observations_point_triangulated != NULL &&
        ...
        if (!(!ctx->problem_selections.do_optimize_intrinsics_core &&
            ...
            // drcal_optimize() and drcal_optimizer_callback()
            assert(0);
        }

#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: drcal_observation_point_t.px is the observation vector in camera coords. No outliers. No intrinsics"
#warning \
    "triangulated-solve: make weights work somehow. This is tied to outliers"
#endif

        for (int i0 = 0; i0 < ctx->Nobservations_point_triangulated; i0++) {
            const drcal_observation_point_triangulated_t* pt0 =
            ...
            if (!pt0->outlier) {
                ...
                // }
                //
                // Doing the triangulation in the frame of point1 allows me to
                // do more stuff in the outer compute_stuff_for_point0
                // computation, and less in the inner compute_stuff_for_point1
                // computation
                ...
                //
                // I don't actually have t_r0: I have t_0r, so I need to compute
                // an inversion. y = R x + t -> x = Rinv y - Rinv t -> tinv =
                // -Rinv t t_r0 = -R_r0 t_0r
                ...
                const int icam_extrinsics0 = pt0->icam.extrinsics;
                if (icam_extrinsics0 >= 0) {
                    const drcal_pose_t* rt_0r = &camera_rt[icam_extrinsics0];
                    const double* r_0r = &rt_0r->r.xyz[0];
                    const double* t_0r = &rt_0r->t.xyz[0];
                ...
                while (true) {
                    const drcal_observation_point_triangulated_t* pt1 =
                    ...
                    if (!pt1->outlier) {
                        ...
                        if (icam_extrinsics1 >= 0) {
                            ...
                            //      = t_r0
                            t_10 = t_r0;
                            // At most one camera can sit at the reference. So
                            // if I'm here, I know that t_r0 != NULL and thus
                            // t_10 != NULL

                            v0_cam1 = v0_ref;
                        }

                        drcal_point3_t derr__dv0_cam1;
                        drcal_point3_t derr__dt_10;

                        double err = _drcal_triangulated_error(
                            &derr__dv0_cam1,
                            &derr__dt_10,
                            v1,
                            v0_cam1,
                        ...
                        if (Jt) {
                            ...
                            if (icam_extrinsics0 >= 0) {
                                ...
                                if (icam_extrinsics1 >= 0) {
                                    ...
                                    // camera1 is at the reference, so I don't
                                    // have dt_10__dt_r0 and dv0_cam1__dv0_ref
                                    // explicitly stored.
                                    //
                                    // t_10    = t_r0   --> dt_10__dt_r0      =
                                    // I v0_cam1 = v0_ref --> dv0_cam1__dv0_ref
                                    // = I
                            ...
                            if (icam_extrinsics1 >= 0) {
                                ...
                                if (icam_extrinsics0 >= 0) {
                                    ...
                                    // camera0 is at the reference.
                                    // dt_10__drt_1r is not given explicitly
                                    //
                                    // t_10 = t_1r ->
                                    //   dt_10__dr_1r = 0
                                    //   dt_10__dt_1r = I
                                    // So
                                    //
                                    //   derr/dr_1r = derr/dv0_cam1
                                    //   dv0_cam1/dr_1r derr/dt_1r = derr/dt_10
                                    SCALE_JACOBIAN_N(
                                        i_var_camera_rt1 + 0,
                                        SCALE_ROTATION_CAMERA,
                                        3
                            ...
                            // Don't need the Jacobian. I just move iJacobian as
                            // needed
                            if (icam_extrinsics0 >= 0) {
                                iJacobian += 6;
                            }
    ...
    ///////////////// Regularization
    {
        ...
        // contributions:
        //
        //   Nmeasurements_rest*normal_pixel_error_sq *
        //   0.005/Nregularization_types =
        //   Nmeasurements_regularization_distortion
        //   *normal_regularization_distortion_error_sq  =
        ...
        //
        //   normal_regularization_distortion_error_sq     =
        //   (scale*normal_distortion_offset )^2
        //   normal_regularization_centerpixel_error_sq    =
        //   (scale*normal_centerpixel_value )^2
        //   normal_regularization_unity_cam01_error_sq    =
        //   (scale*normal_unity_cam01_value )^2
        //
        ...
        int Nmeasurements_nonregularization =
            ctx->Nmeasurements - (Nmeasurements_regularization_distortion +
                                  Nmeasurements_regularization_centerpixel +
        ...
        if (dump_regularizaton_details) {
            MSG("expected_total_pixel_error_sq: %f",
                expected_total_pixel_error_sq);
        }

        // This is set to 2 to match what drcal 2.4 does, to keep the behavior
        ...
        if (ctx->problem_selections.do_apply_regularization &&
            ...
            // compute scales
            {
                if (ctx->problem_selections
                    ...
                    // other models I use 2.0 because that's what I had for a
                    // long time, and I don't want to change it to not break
                    // anything
                    ...
                    double expected_regularization_distortion_error_sq_noscale =
                        ...
                        normal_distortion_value * normal_distortion_value;

                    double scale_sq =
                        expected_total_pixel_error_sq * 0.005 /
                        (double)Nregularization_types /
                        expected_regularization_distortion_error_sq_noscale;

                    if (dump_regularizaton_details) {
                        MSG("expected_regularization_distortion_error_sq: %f",
                            expected_regularization_distortion_error_sq_noscale *
                                scale_sq);
                    }
                ...
                if (modelHasCore_fxfycxcy(&ctx->lensmodel) &&
                    ...
                    double
                        expected_regularization_centerpixel_error_sq_noscale =
                    ...
                    if (dump_regularizaton_details) {
                        MSG("expected_regularization_centerpixel_error_sq: %f",
                            expected_regularization_centerpixel_error_sq_noscale *
                                scale_sq);
                    }
            ...
            // compute and store regularization terms
            {
                if (ctx->problem_selections
                    ...
                    for (int icam_intrinsics = 0;
                        ...
                        if (ctx->lensmodel.type ==
                            drcal_LENSMODEL_SPLINED_STEREOGRAPHIC) {
                            // Splined model regularization. I do directional L2
                            // regularization. At each knot I penalize
                            // contributions in the tangential direction much
                            // more than in the radial direction. Otherwise
                            // noise in the data produces lots of curl in the
                            // vector field. This isn't wrong, but it's much
                            // nicer if "right" in the camera coordinate system
                            // corresponds to "right" in pixel space
                            const int Nx =
                                ctx->lensmodel
                                    .LENSMODEL_SPLINED_STEREOGRAPHIC__config.Nx;
                            const int Ny =
                                ctx->lensmodel
                                    .LENSMODEL_SPLINED_STEREOGRAPHIC__config.Ny;

                            for (int iy = 0; iy < Ny; iy++) {
                                for (int ix = 0; ix < Nx; ix++) {
                                    double scale =
                                        scale_regularization_distortion;
                                    ...
                                    // WARNING: "Precompute uxy. This is lots of
                                    // unnecessary computation in the inner
                                    // loop"
                                    double uxy[] = {
                                        (double)(2 * ix - Nx + 1),
                                        (double)(2 * iy - Ny + 1)
                                    ...
                                    norm2_error += err * err;
                                    STORE_JACOBIAN(
                                        i_var_intrinsics + Ncore_state + ivar +
                                            0,
                            ...
                            for (int j = 0; j < ctx->Nintrinsics - Ncore; j++) {
                                // This maybe should live elsewhere, but I put
                                // it here for now. Various distortion
                                // coefficients have different meanings, and
                                // should be regularized in different ways.
                                // Specific logic follows
                                double scale = scale_regularization_distortion;

                                if (drcal_LENSMODEL_IS_OPENCV(
                                        ctx->lensmodel.type
                                    ) &&
                                    ctx->lensmodel.type >=
                                        drcal_LENSMODEL_OPENCV8 &&
                                    5 <= j && j <= 7) {
                                    // The radial distortion in opencv is
                                    // x_distorted = x*scale where r2 = norm2(xy
                                    // - xyc) and
                                    //
                                    // scale = (1 + k0 r2 + k1 r4 + k4 r6)/(1 +
                                    // k5 r2 + k6 r4 + k7 r6)
                                    //
                                    // Note that k2,k3 are tangential (NOT
                                    // radial) distortion components. Note that
                                    // the r6 factor in the numerator is only
                                    // present for
                                    // >=drcal_LENSMODEL_OPENCV5. Note that the
                                    // denominator is only present for >=
                                    // drcal_LENSMODEL_OPENCV8. The danger with
                                    // a rational model is that it's possible to
                                    // get into a situation where scale ~ 0/0
                                    // ~ 1. This would have very poorly behaved
                                    // derivatives. If all the rational
                                    // coefficients are ~0, then the denominator
                                    // is always ~1, and this problematic case
                                    // can't happen. I favor that by
                                    // regularizing the coefficients in the
                                    // denominator more strongly
                                    scale *= 5.;
                                }
                                ...
                                double err =
                                    scale *
                                    intrinsics_all[icam_intrinsics][j + Ncore];
                                x[iMeasurement] = err;
                                norm2_error += err * err;

                                STORE_JACOBIAN(
                                    i_var_intrinsics + Ncore_state + j,
                                    scale * SCALE_DISTORTION
                                );

                                iMeasurement++;
                                if (dump_regularizaton_details) {
                                    MSG("regularization distortion: %g; norm2: "
                                        "%g",
                                        err,
                                        err * err);
                                }
                ...
                if (modelHasCore_fxfycxcy(&ctx->lensmodel) &&
                    ...
                    for (int icam_intrinsics = 0;
                        ...
                        // And another regularization term: optical center
                        // should be near the middle. This breaks the symmetry
                        // between moving the center pixel coords and
                        // pitching/yawing the camera.
                        double cx_target =
                            0.5 *
                            (double)(ctx->imagersizes[icam_intrinsics * 2 + 0] -
                                     1);
                        double cy_target =
                            0.5 *
                            (double)(ctx->imagersizes[icam_intrinsics * 2 + 1] -
                                     1);
                        ...
                        STORE_JACOBIAN(
                            i_var_intrinsics + 2,
                            scale_regularization_centerpixel *
                                SCALE_INTRINSICS_CENTER_PIXEL
                        );
                        iMeasurement++;
                        if (dump_regularizaton_details) {
                            MSG("regularization center pixel off-center: %g; "
                                "norm2: %g",
                                err,
                                err * err);
                        }
...
#warning "triangulated-solve: better unity_cam01 scale"
                ...
                double expected_regularization_unity_cam01_error_sq_noscale =
                    ...
                    normal_unity_cam01_value * normal_unity_cam01_value;
                ...
                if (dump_regularizaton_details) {
                    MSG("expected_regularization_unity_cam01_error_sq: %f",
                        expected_regularization_unity_cam01_error_sq_noscale *
                            scale_sq);
                }
            ...
            // compute and store regularization terms
            {
                ...
                // reference is t_0r
                const drcal_point3_t* t_0r = &camera_rt[0].t;

                const int i_var_extrinsics = drcal_state_index_extrinsics(
                    0,
                    ctx->Ncameras_intrinsics,
                    ctx->Ncameras_extrinsics,
                    ctx->Nframes,
                ...
                double err = scale_regularization_unity_cam01 *
                ...
                for (int i = 0; i < 3; i++) {
                    STORE_JACOBIAN(
                        i_var_extrinsics + 3 + i,
                        scale_regularization_unity_cam01 *
                            SCALE_TRANSLATION_CAMERA * 2. * t_0r->xyz[i]
                    );
                }

                iMeasurement++;
                if (dump_regularizaton_details) {
                    MSG("regularization unity_cam01: %g; norm2: %g",
                        err,
                        err * err);
                }
            }
...
bool drcal_optimizer_callback(
    // out

    // These output pointers may NOT be NULL, unlike
    // their analogues in drcal_optimize()
    ...
    if ((observations_point_triangulated != NULL &&
        ...
        !(!problem_selections.do_optimize_intrinsics_core &&
          !problem_selections.do_optimize_intrinsics_distortions &&
          problem_selections.do_optimize_extrinsics)) {
        MSG("ERROR: We have triangulated points. At this time this is only "
            "allowed if we're NOT optimizing intrinsics AND if we ARE "
    ...
    if (Nobservations_board > 0) {
        if (problem_selections.do_optimize_calobject_warp &&
            calobject_warp == NULL) {
            MSG("ERROR: We're optimizing the calibration object warp, so a "
                "buffer with a seed MUST be passed in.");
            goto done;
        }
    } else {
    ...
    if (buffer_size_b_packed != Nstate * (int)sizeof(double)) {
        MSG("The buffer passed to fill-in b_packed has the wrong size. Needed "
            "exactly %d bytes, but got %d bytes",
            Nstate * (int)sizeof(double),
            buffer_size_b_packed);
        goto done;
    }

    int Nmeasurements = drcal_num_measurements(
        Nobservations_board,
        Nobservations_point,
        observations_point_triangulated,
        Nobservations_point_triangulated,
    ...
    int Nintrinsics = drcal_lensmodel_num_params(lensmodel);
    int N_j_nonzero = _drcal_num_j_nonzero(
        Nobservations_board,
        Nobservations_point,
        observations_point_triangulated,
        Nobservations_point_triangulated,
    ...
    if (buffer_size_x != Nmeasurements * (int)sizeof(double)) {
        MSG("The buffer passed to fill-in x has the wrong size. Needed exactly "
            "%d bytes, but got %d bytes",
            Nmeasurements * (int)sizeof(double),
            buffer_size_x);
        goto done;
    }
    ...
    const callback_context_t ctx = {
        ...
        .calibration_object_width_n =
            calibration_object_width_n > 0 ? calibration_object_width_n : 0,
        .calibration_object_height_n =
            calibration_object_height_n > 0 ? calibration_object_height_n : 0,
        .Nmeasurements = Nmeasurements,
        .N_j_nonzero = N_j_nonzero,
        .Nintrinsics = Nintrinsics
    };
    _drcal_precompute_lensmodel_data(
        (drcal_projection_precomputed_t*)&ctx.precomputed,
        lensmodel
    );
...
drcal_stats_t drcal_optimize(
    ...
    drcal_pose_t*
        frames_toref,  // Nframes of these.    Transform TO the reference frame
    drcal_point3_t* points,  // Npoints of these.    In the reference frame
    ...
    //
    // ( Nobservations_point,)
    drcal_point3_t* observations_point_pool,
    ...
    if (Nobservations_board > 0) {
        if (problem_selections.do_optimize_calobject_warp &&
            ...
            MSG("ERROR: We're optimizing the calibration object warp, so a "
                "buffer with a seed MUST be passed in.");
            return (drcal_stats_t){.rms_reproj_error__pixels = -1.0};
        }
    } else {
        problem_selections.do_optimize_calobject_warp = false;
    }
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: can I loosen this? Optimizing intrinsics and triangulated points together should work"
#endif
    }
    ...
    callback_context_t ctx = {
        ...
        .calibration_object_height_n =
            calibration_object_height_n > 0 ? calibration_object_height_n : 0,
        .Nmeasurements = drcal_num_measurements(
            Nobservations_board,
            Nobservations_point,
            observations_point_triangulated,
            Nobservations_point_triangulated,
        ...
        .N_j_nonzero = _drcal_num_j_nonzero(
            Nobservations_board,
            Nobservations_point,
            observations_point_triangulated,
            Nobservations_point_triangulated,
        ...
        .Nintrinsics = drcal_lensmodel_num_params(lensmodel)
    };
    _drcal_precompute_lensmodel_data(
        (drcal_projection_precomputed_t*)&ctx.precomputed,
        lensmodel
    ...
    if (b_packed_final != NULL &&
        ...
        MSG("The buffer passed to fill-in b_packed_final has the wrong size. "
            "Needed exactly %d bytes, but got %d bytes",
            Nstate * (int)sizeof(double),
            buffer_size_b_packed_final);
        return (drcal_stats_t){.rms_reproj_error__pixels = -1.0};
    }
    if (x_final != NULL &&
        buffer_size_x_final != ctx.Nmeasurements * (int)sizeof(double)) {
        MSG("The buffer passed to fill-in x_final has the wrong size. Needed "
            "exactly %d bytes, but got %d bytes",
            ctx.Nmeasurements * (int)sizeof(double),
            buffer_size_x_final);
        return (drcal_stats_t){.rms_reproj_error__pixels = -1.0};
    }
    ...
    if (ctx.Nmeasurements <= Nstate) {
        MSG("WARNING: problem isn't overdetermined: Nmeasurements=%d, "
            "Nstate=%d. Solver may not converge, and if it does, the results "
            "aren't reliable. Almost certainly you're about to see warnings "
            "about the system not being positive-definite. Add more "
            "constraints and/or regularization",
            ctx.Nmeasurements,
            Nstate);
    }
    ...
    drcal_stats_t stats = {.rms_reproj_error__pixels = -1.0};

    if (!check_gradient) {
        stats.Noutliers_board = 0;
        stats.Noutliers_triangulated_point = 0;

        const int Nmeasurements_board = drcal_num_measurements_boards(
            Nobservations_board,
            calibration_object_width_n,
            calibration_object_height_n
        );
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: check for point outliers here as well. Pull the triangulated-point outlier code out of markOutliers, and call it here (divergent-ray finder and looking at marked outliers)"
#endif
        ...
        do {
            ...
            norm2_error = dogleg_optimize2(
                ...
                ctx.Nmeasurements,
                ctx.N_j_nonzero,
                (dogleg_callback_t*)&optimizer_callback,
                &ctx,
                &dogleg_parameters,
                &solver_context
            );
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: not const for now. this will become const once the outlier bit moves to the point_triangulated pool"
#endif
...
#endif
                 ({
                     MSG("Threw out some outliers. New count = %d/%d (%.1f%%). "
                         "Going again",
                         stats.Noutliers_board,
                         Nmeasurements_board,
                         (double)(stats.Noutliers_board * 100) /
                             (double)Nmeasurements_board);
                     true;
                 }));
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: the above print should deal with triangulated points too"
#endif

        // Done. I have the final state. I spit it back out
        unpack_solver_state(
            intrinsics,          // Ncameras_intrinsics of these
        ...
        const int imeas_reg0 = drcal_measurement_index_regularization(
            observations_point_triangulated,
            Nobservations_point_triangulated,
            calibration_object_width_n,
            calibration_object_height_n,
        ...
        if (imeas_reg0 >= 0 && verbose) {
            ...
            if (regularization_ratio_distortion > 0.01) {
                MSG("WARNING: regularization ratio for lens distortion exceeds "
                    "1%%. Is the scale factor too high? Ratio = %.3g/%.3g = "
                    "%.3g",
            ...
            if (regularization_ratio_centerpixel > 0.01) {
                MSG("WARNING: regularization ratio for the projection "
                    "centerpixel exceeds 1%%. Is the scale factor too high? "
                    "Ratio = %.3g/%.3g = %.3g",
            ...
            if (problem_selections.do_apply_regularization_unity_cam01 &&
                ...
                for (int i = 0; i < Nmeasurements_regularization_unity_cam01;
                     i++) {
                    double x = *(xreg++);
                ...
                regularization_ratio_unity_cam01 =
                    norm2_err_regularization_unity_cam01 / norm2_error;
                if (regularization_ratio_unity_cam01 > 0.01) {
                    MSG("WARNING: regularization ratio for unity_cam01 exceeds "
                        "1%%. Is the scale factor too high? Ratio = %.3g/%.3g "
                        "= %.3g",
                        norm2_err_regularization_unity_cam01,
                        norm2_error,
                        regularization_ratio_unity_cam01);
                }
            }
            ...
            // {
            //     double x = solver_context->beforeStep->x[ctx.Nmeasurements -
            //     Nmeasurements_regularization + i]; MSG("regularization %d: %f
            //     (squared: %f)", i, x, x*x);
            // }
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: not correct for triangulated points either: 1 measurement, not 2"
#endif
        // THIS IS NOT CORRECT FOR POINTS. I have 3 measurements for points
        // currently
    ...
    if (x_final) {
        memcpy(
            x_final,
            solver_context->beforeStep->x,
            ctx.Nmeasurements * sizeof(double)
        );
    }
...
bool drcal_write_cameramodel_file(
    const char* filename,
    const drcal_cameramodel_t* cameramodel
) {
    bool result = false;
    ...
    if (!drcal_lensmodel_name(
            lensmodel_string,
            sizeof(lensmodel_string),
            &cameramodel->lensmodel
        )) {
        MSG("Couldn't construct lensmodel string. Unconfigured string: '%s'",
            drcal_lensmodel_name_unconfigured(&cameramodel->lensmodel));
        goto done;
    }

    int Nparams = drcal_lensmodel_num_params(&cameramodel->lensmodel);
    if (Nparams < 0) {
...
#if defined ENABLE_TRIANGULATED_WARNINGS && ENABLE_TRIANGULATED_WARNINGS
#warning \
    "triangulated-solve: fixed points should live in a separate array, instead of at the end of the 'points' array"
#endif
