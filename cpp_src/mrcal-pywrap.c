...
static PyObject* load_image(
    ...
    char* keywords[] = {"filename", "bits_per_pixel", "channels", NULL};
    if (!PyArg_ParseTupleAndKeywords(
            args,
            kwargs,
    ...
    if ((bits_per_pixel <= 0 && channels > 0) ||
        ...
        BARF(
            "Both bits_per_pixel and channels should be given valid values >0, "
    ...
    if (bits_per_pixel <= 0) {
        if (!mrcal_image_anytype_load(
            ...
            BARF(
                "Error loading image '%s' with autodetected "
    ...
    if (bits_per_pixel == 8 && channels == 1) {
        if (image.data == NULL &&
            !mrcal_image_uint8_load((mrcal_image_uint8_t*)&image, filename)) {
            BARF("Error loading image '%s'", filename);
            goto done;
        }
        image_array =
            PyArray_SimpleNew(2, ((npy_intp[]){image.h, image.w}), NPY_UINT8);
    } else if (bits_per_pixel == 16 && channels == 1) {
        if (image.data == NULL &&
            !mrcal_image_uint16_load((mrcal_image_uint16_t*)&image, filename)) {
            BARF("Error loading image '%s'", filename);
            goto done;
        }
        image_array =
            PyArray_SimpleNew(2, ((npy_intp[]){image.h, image.w}), NPY_UINT16);
    } else if (bits_per_pixel == 24 && channels == 3) {
        if (image.data == NULL &&
            !mrcal_image_bgr_load((mrcal_image_bgr_t*)&image, filename)) {
            BARF(
                "Error loading image '%s' with bits_per_pixel=%d and "
                "channels=%d",
                filename,
                bits_per_pixel,
                channels
        ...
        BARF(
            "Unsupported format requested. I only support "
            "(bits_per_pixel,channels) = (8,1) and (16,1) and (24,3)"
        );
        goto done;
    }
    ...
    for (int i = 0; i < image.h; i++) {
        memcpy(
            &((uint8_t*)PyArray_DATA((PyArrayObject*)image_array)
            )[image.w * bits_per_pixel / 8 * i],
...
static PyObject* save_image(
    ...
    char* keywords[] = {"filename", "array", NULL};
    if (!PyArg_ParseTupleAndKeywords(
            args,
            kwargs,
            "sO:mrcal.save_image",
    ...
    if (ndim == 2 && dtype == NPY_UINT8) {
        if (strides[ndim - 1] != 1) {
            BARF(
                "Saving 8-bit grayscale array. I only know how to handle "
                "stride[-1] == 1"
            );
            goto done;
        }
        ...
        if (strides[ndim - 1] != 2) {
            BARF(
                "Saving 16-bit grayscale array. I only know how to handle "
                "stride[-1] == 2"
            );
            goto done;
        }
        ...
        if (dims[2] != 3) {
            BARF(
                "Saving 3-dimensional array. shape[-1] != 3, so not BGR. Don't "
                "know what to do"
            );
            goto done;
        }

        if (strides[ndim - 1] != 1 || strides[ndim - 2] != 3) {
            BARF(
                "Saving 8-bit BGR array. I only know how to handle stride[-1] "
                "== 1 && stride[-2] == 3"
            );
            goto done;
        }
        ...
        BARF(
            "Unsupported array. I only support (bits_per_pixel,channels) = "
            "(8,1) and (16,1) and (24,3)"
        );
        goto done;
    }
...
static const char load_image_docstring[] = R"(
...
static const char save_image_docstring[] = R"(
